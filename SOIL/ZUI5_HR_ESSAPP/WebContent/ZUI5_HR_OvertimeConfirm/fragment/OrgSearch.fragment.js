sap.ui.jsfragment("ZUI5_HR_OvertimeConfirm.fragment.OrgSearch", {

	createContent : function(oController) {	
		
		jQuery.sap.require("ZUI5_HR_OvertimeConfirm.control.OrgSearch");
		...
		var oFilterBar = new sap.m.Toolbar({
			content : [ 
				...
				new sap.m.Input(oController.PAGEID + "_OrgSearch_Stext", {
					width: "150px"
				}).attachBrowserEvent("keyup", ZUI5_HR_OvertimeConfirm.control.OrgSearch.onKeyUp),
				new sap.m.ToolbarSpacer({width: "10px"}),
				new sap.m.Button(oController.PAGEID + "_OrgSearch_SearchButton", {
					...
					press : ZUI5_HR_OvertimeConfirm.control.OrgSearch.OrgSearch
				}),
				new sap.m.ToolbarSpacer({width: "10px"})
			]
		...
		var oDialog = new sap.m.Dialog(oController.PAGEID + "_OrgSearch_Dialog", {
			...
			showHeader : true,
			title : "{Title}",
			beforeOpen : ZUI5_HR_OvertimeConfirm.control.OrgSearch.onBeforeOpenOrgSearchDialog,
			afterOpen : ZUI5_HR_OvertimeConfirm.control.OrgSearch.onAfterOpenOrgSearchDialog,
			afterClose : ZUI5_HR_OvertimeConfirm.control.OrgSearch.onAfterCloseOrgSearchDialog,
			beginButton :  new sap.m.Button({
				icon : "sap-icon://accept",
				text : oBundleText.getText("LABEL_0037"),	// 37:선택
				press : ZUI5_HR_OvertimeConfirm.control.OrgSearch.onConfirm}),
			endButton : new sap.m.Button({
				icon : "sap-icon://sys-cancel-2",
				text :  oBundleText.getText("LABEL_0071"),	// 71:취소
				press : ZUI5_HR_OvertimeConfirm.control.OrgSearch.onClose}),
			content : [oLayout]
		}).setModel(new sap.ui.model.json.JSONModel())	
		.bindElement("/Data");
