...
sap.ui.controller("ZUI5_HR_TimeReport.ZUI5_HR_TimeReportList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_TimeReport.ZUI5_HR_TimeReportList
	 */

	PAGEID : "ZUI5_HR_TimeReportList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {
			...
			var JSonData = { Data : { Begda : dateFormat.format(prevDate) , Endda : dateFormat.format(curDate), Encid : vEncid, Ename : vEname, Auth : _gAuth, Comcd : "" , Orgeh : "" }};
	...
	onPressSearch : function(oEvent) {
		...
		if(SerchCond.Comcd != "") {
			aFilters.push(new sap.ui.model.Filter('Comcd', sap.ui.model.FilterOperator.EQ, SerchCond.Comcd));
		}
		if(SerchCond.Encid != "") {
		...
		function Search() {
			...
			if(Datas.Data.length == 1 && Datas.Data[0].Orgtx == "합계") {
				Datas.Data = [];
			}
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		if(oJSONModelData.length >= 1000) {
			// CSV Export
			common.Common.exportToExcel(oTable, oController, oBundleText.getText("LABEL_2935") + "-" + dateFormat.format(curDate));	// 2935:근태/휴가내역조회
		} else {
			var oSettings = {
					workbook: { columns: oController._Columns },
					dataSource: oJSONModelData,
					worker: false, // We need to disable worker because we are using a MockServer as OData Service
					fileName: oBundleText.getText("LABEL_2935") + "-" + dateFormat.format(curDate) + ".xlsx"	// 2935:근태/휴가내역조회
			};
	...
	displayOrgehSearchDialog : function(oEvent) {
		...
		if(!oController._SerachOrgDialogInView) {
			oController._SerachOrgDialogInView = sap.ui.jsfragment("ZUI5_HR_TimeReport.fragment.OrgSearch", oController);
