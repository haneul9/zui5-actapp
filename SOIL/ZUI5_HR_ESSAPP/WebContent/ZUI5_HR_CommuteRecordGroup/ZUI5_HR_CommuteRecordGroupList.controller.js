...
sap.ui.controller("ZUI5_HR_CommuteRecordGroup.ZUI5_HR_CommuteRecordGroupList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_CommuteRecordGroup.ZUI5_HR_CommuteRecordGroupList
	 */

	PAGEID : "ZUI5_HR_CommuteRecordGroupList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			if((_gAuth == "E" || _gAuth == "M") && vEmpLoginInfo.length > 0) {
			...
			var JSonData = { Data : { Tsmon : dateFormat.format(vTsmon), Encid : vEncid, Ename : vEname, Finyn : "ALL", Auth : _gAuth }};
	...
	onSelectRow : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroup.ZUI5_HR_CommuteRecordGroupList"),
	...
	onClubDblclick : function(e){
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroup.ZUI5_HR_CommuteRecordGroupList"),
			oController = oView.getController(),
			oTableTxt = sap.ui.getCore().byId(oController.PAGEID + "_TableTxt"),
			selectRowObject = e.getParameter('rowContext').getObject();
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0703") + "-" + dateFormat.format(curDate) + ".xlsx"	// 703:근태기록부확인(부서장)
		};
