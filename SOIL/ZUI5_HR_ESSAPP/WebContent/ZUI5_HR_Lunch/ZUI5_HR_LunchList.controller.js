...
sap.ui.controller("ZUI5_HR_Lunch.ZUI5_HR_LunchList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_Lunch.ZUI5_HR_LunchList
	 */

	PAGEID : "ZUI5_HR_LunchList",
...
//		};
	...
	onDetailPage : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Lunch.ZUI5_HR_LunchList");
		var oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_Lunch.ZUI5_HR_LunchDetail",
		      data : {
		    	  Appno : _Appno ,
		    	  Famgb : "",
		      }
	...
	onPressSearch : function(oEvent){
		...
		var oPath = "/LunchFeeExpensesApplSet";
		var aFilters = [];
		...
		if(SerchCond.Payym != ""){
			aFilters.push(new sap.ui.model.Filter('Payym', sap.ui.model.FilterOperator.EQ, SerchCond.Payym));
		}
		if(SerchCond.Lnfty != ""){
			aFilters.push(new sap.ui.model.Filter('Lnfty', sap.ui.model.FilterOperator.EQ, SerchCond.Lnfty));
		}
				...
				success : function(data,res){
					if(data && data.results.length){
						for(var i=0;i<data.results.length;i++){
							...
							OneData.Reqym = OneData.Reqym == "000000" ? "" :  OneData.Reqym.substring(0,4) + "." + OneData.Reqym.substring(4,6) ; 
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_1383") + "-" + dateFormat.format(curDate) + ".xlsx"	// 1383:중식비 예외자 신청
			};
