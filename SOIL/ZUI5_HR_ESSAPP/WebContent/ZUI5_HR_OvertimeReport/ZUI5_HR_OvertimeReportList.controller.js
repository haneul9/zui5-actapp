...
sap.ui.controller("ZUI5_HR_OvertimeReport.ZUI5_HR_OvertimeReportList", {
	/**
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {
			...
			var JSonData = { Data : { Begda : dateFormat.format(prevDate) , Endda : dateFormat.format(curDate), Encid : vEncid, Ename : vEname, Auth : _gAuth }};
	...
	onPressSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeReport.ZUI5_HR_OvertimeReportList"),
		...
		function Search() {
			...
			oModel.read("/SpecialWorkStatusSet", {
				async: false,
				filters: aFilters,
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		if(oJSONModelData.length >= 1000) {
			// CSV Export
			common.Common.exportToExcel(oTable, oController, oBundleText.getText("LABEL_2864") + "-" + dateFormat.format(curDate));	// 2864:특근조회(개인별)
		} else {
			var oSettings = {
					workbook: { columns: oController._Columns },
					dataSource: oJSONModelData,
					worker: false, // We need to disable worker because we are using a MockServer as OData Service
					fileName: oBundleText.getText("LABEL_2864") + "-" + dateFormat.format(curDate) + ".xlsx"	// 2864:특근조회(개인별)
			};
