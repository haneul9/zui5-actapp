...
//				this.onBeforeShow(evt);
			...
			if(_gZmcode != ""){
				...
				for(var i =0; i < oController._EssMenuList.length; i++){
					if(oController._EssMenuList[i].Mncod == _gZmcode){
						vExist = "X";
						if(oController._EssMenuList[i].Pwuse == true){
							common.Password.openPasswordDialog(oController, _gZmcode);
						}else{
	...
	onSetMenu : function(oController){
		...
		navContainer.addPage(
			new sap.m.ScrollContainer("E_Home",{
				vertical: true,
				height : "100%",
				horizontal: false,
				content: [
		...
		// Home 화면 추가
		navContainer.addPage(
			new sap.m.ScrollContainer("M_Home",{
				vertical: true,
				height : "100%",
				horizontal: false,
				content: [
		...
		// Home 화면 추가
		navContainer.addPage(
			new sap.m.ScrollContainer("H_Home",{
				vertical: true,
				height : "100%",
				horizontal: false,
				content: [
		...
		navContainer.addPage(
			new sap.m.ScrollContainer("E_Apply",{
				vertical: false,
				height : "100%",
				horizontal: false,
				content: [
		...
		navContainer.addPage(
			new sap.m.ScrollContainer("E_Approval",{
				vertical: false,
				height : "100%",
				horizontal: false,
				content: [
		...
		navContainer.addPage(
			new sap.m.ScrollContainer("H_Approval",{
				vertical: false,
				height : "100%",
				horizontal: false,
				content: [
		...
		for(var i = 0; i < oController._MenuList.length; i++){
			...
			vTemp.Popup = oController._MenuList[i].Popup;
			oController._SearchControlPage.push(vTemp);
			
			if(oController._MenuList[i].Popup == ""){
				// Navigation Page 등록
				navContainer.addPage(
	...
	onPressLev1 : function(oEvent, rAuth){
		...
		oController.onClearNavigation(oController, Auth + "_Home");
	...
	searchEmployee : function(oController){
		  ...
		  var filterString = "?$filter=Actda%20eq%20datetime%27" + dateFormat.format(curDate) + "T00%3a00%3a00%27" 
			   ...
			   filterString += " and ReqAuth eq '1'";	// 권한 추가 
...
//								                   Stetx : oData.results[0].Stetx ,
	...
	onSearch : function(event){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Portal.ZUI5_HR_PortalList");
		var oController = oView.getController(), vZurl = "";
		...
		if (item && item.getKey()) {
			for(var i =0; i< oController._SearchControlPage.length; i++){
				if(item.getKey() == oController._SearchControlPage[i].key){
					...
					if(oController._SearchControlPage[i].Popup == "X")  
						vZurl = oController._SearchControlPage[i].url;
					if(oController._SearchControlPage[i].Pwuse == true){
						...
						oController.goToPage(oController, item.getKey(), vZurl);
						oController.onClearNavigation(oController);
					}
					break;
				}
	...
	goToPage : function(oController, PageKey, NewWindowUrl ){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Portal.ZUI5_HR_PortalList");
		var oController = oView.getController();
		var navContainer = sap.ui.getCore().byId("navContainer");
