...
sap.ui.controller("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail
	 */

	PAGEID : "ZUI5_HR_OvertimeConfirmMultiDetail",
	...
	onAfterShow : function(oEvent) {
		...
		common.Common.generateRowspan({
			selector : '#ZUI5_HR_OvertimeConfirmMultiDetail_Table-header-fixed-fixrow > tbody',
			colIndexes : [0, 1]
		});
		
		common.Common.generateRowspan({
			selector : '#ZUI5_HR_OvertimeConfirmMultiDetail_Table-header > tbody',
			colIndexes : [7, 8, 16, 17, 24, 25]
		});
	},
	
	onBack : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : (oController._vFromPage) ? oController._vFromPage : "ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiList",
			data : { }
		});
	},
...
//		oTargetMatrix.removeAllRows();
	...
	setEndTimeFormat : function(oEvent) {
		...
		oController._DetailTableJSonModel.setProperty(_selPath + "/Outimt", undefined);
	...
	onResetTime : function(oEvent) {
		...
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim11", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim12", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim13", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim14", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim21", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Tim22", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Wtm01", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Wtm02", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Wtm03", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Wtme1", undefined);
		oController._DetailTableJSonModel.setProperty("/Data/" + vIdx + "/Wtme2", undefined);
	},
	...
	onPressCheckWorktime : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail"),
			oController = oView.getController(),
			oModel = sap.ui.getCore().getModel("ZHR_SPECIAL_WORK_SRV"),
			vTableDatas = oController._DetailTableJSonModel.getProperty("/Data"),
			isTimeInputValid = true;
		
		if(vTableDatas.length < 1) return;
		
		vTableDatas.forEach(function(elem) {
			if(!elem.Beguzt || !elem.Enduzt) isTimeInputValid = false;
		});
		...
		vTableDatas.forEach(function(element) {
			vDetailData = common.Common.copyByMetadata(oModel, "CfmSpecialWorkDetail", element);
			vDetailData.Datum = "\/Date("+ common.Common.getTime(vDetailData.Datum)+")\/";
			vDetailData.Beguzt = vDetailData.Beguzt.replace(/[^\d]/g, '');
			vDetailData.Enduzt = vDetailData.Enduzt.replace(/[^\d]/g, '');
			
			vDetailDataList.push(vDetailData);
		});
		
		cData.CfmSpecialWorkNav = vDetailDataList;
		
		oModel.create("/CfmSpecialWorkListSet", cData, {
			success: function(data, res) {
				if(data.CfmSpecialWorkNav.results && data.CfmSpecialWorkNav.results.length) {
					data.CfmSpecialWorkNav.results.forEach(function(elem, index, array) {
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim11", elem.Tim11);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim12", elem.Tim12);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim13", elem.Tim13);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim14", elem.Tim14);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim21", elem.Tim21);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Tim22", elem.Tim22);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm01", elem.Wtm01);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm02", elem.Wtm02);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm03", elem.Wtm03);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm04", elem.Wtm04);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm05", elem.Wtm05);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm06", elem.Wtm06);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtm07", elem.Wtm07);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtme1", elem.Wtme1);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wtme2", elem.Wtme2);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Wrkjobt", elem.Wrkjobt);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Twrkjobt", elem.Wrkjobt);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Gigan1", elem.Gigan1);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Gigan2", elem.Gigan2);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Stat1", elem.Stat1);
						oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Stat2", elem.Stat2);
					});
				}
			},
			error: function (Res) {
	...
	onPressSelectByWorktime : function(oEvent) {
		...
		if(!oController._OvertimeDialog) {
			oController._OvertimeDialog = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.OvertimeDialog", oController);
	...
	searchOvertime : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail"),
			...
			vPernr = oOvertimeDialog.getModel().getProperty("/Data/Pernr"),
			vTableDatas = {Data : []},
			vIdx = 1;
	...
	onConfirmOvertimeDialog : function(oEvent) {
		...
		if(vIdxs.length < 1) {
			...
			var preDatumStartOfWeek = "",
				curDatumStartOfWeek = "",
				isSameWeekValid = true;
			
			vIdxs.forEach(function(idx) {
				var _sPath = oOvertimeTable.getContextByIndex(idx).sPath,
					curDatum = mOvertimeTableModel.getProperty(_sPath).Datum;
				
				// 선택 일자의 주차 시작일
				curDatumStartOfWeek = moment(curDatum).startOf('week');
				
				if(preDatumStartOfWeek != "" && !curDatumStartOfWeek.isSame(preDatumStartOfWeek)) {
					isSameWeekValid = false;
				}
				
				preDatumStartOfWeek = curDatumStartOfWeek;
			});
			
			if(!isSameWeekValid) {
				sap.m.MessageBox.alert(oBundleText.getText("LABEL_2919"));	// 2919:동일 주의 명령서만 선택합니다.
				return;
			}
		}
		...
		vIdxs.forEach(function(idx, index, array) {
			var sData = mOvertimeTableModel.getProperty(oOvertimeTable.getContextByIndex(idx).sPath),
				copyData = {};
			
			copyData.Idx = index + 1;
			copyData.Oappno = sData.Appno;
			copyData.Pernr = sData.Pernr;
			copyData.Perid = sData.Perid;
			copyData.Ename = sData.Ename;
			copyData.Orgtx = vOrgtx;
...
//			copyData.Otim22 = sData.Tim22;
			...
			copyData.Outimt = sData.Outimt.replace(/(\d{2})(\d{2})/g, '$1:$2');
			
			copyData.Appno = vAppno;
			copyData.ZappStatAl = vZappStatAl;
			copyData.Persa = vPersa;
			
			vCopyDataList.push(copyData);
		});
		
		oController._DetailTableJSonModel.setProperty("/Data", vCopyDataList);
		oTable.setVisibleRowCount(vCopyDataList.length || 1);
	...
	onPressIntimeDialog : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail"),
			oController = oView.getController(),
			oTable = sap.ui.getCore().byId(oController.PAGEID + "_Table"),
			mTableModel = oTable.getModel(),
			vSeqno = oEvent.getSource().getCustomData()[0].getValue(),
			vIdx = vSeqno * 1 - 1,
			oRowData = mTableModel.getProperty("/Data/" + vIdx),
			vDatum = oRowData.Datum,
			vBeguzt = oRowData.Beguzt,
			vEnduzt = oRowData.Enduzt,
			vPernr = oRowData.Pernr;
		...
		if(!oController._IntimeDialog) {
			oController._IntimeDialog = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.IntimeDialog", oController);
	...
	onPressOuttimeDialog : function(oEvent) {
		...
		if(!oController._OuttimeDialog) {
			oController._OuttimeDialog = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.OuttimeDialog", oController);
	...
	onConfirmIntimeDialog : function(oEvent) {
		...
		oController._DetailTableJSonModel.setProperty("/Data/" + oController._selectionRowIdx + "/Intimt", sData.Intimt.replace(/(\d{2})(\d{2})/g, '$1:$2'));
	...
	onConfirmOuttimeDialog : function(oEvent) {
		...
		oController._DetailTableJSonModel.setProperty("/Data/" + oController._selectionRowIdx + "/Outimt", sData.Outimt.replace(/(\d{2})(\d{2})/g, '$1:$2'));
	...
	displayOrgSearchDialog : function(oEvent) {
		...
		if(!oController._SerachOrgDialogInView) {
			oController._SerachOrgDialogInView = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.OrgSearch", oController);
	...
	onRowPernrChange : function(oEvent) {
		...
		if(!oController._PernrDialog) {
			oController._PernrDialog = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.PernrDialog", oController);
	...
	commonAction : function(oController, oDetailData) {
		...
		if(vPersa == "3000") {
			oController._DetailTableJSonModel.getData().Data.forEach(function(elem, index, array) {
				oController._DetailTableJSonModel.setProperty("/Data/" + index + "/Persa", vPersa);
			});
		}
	...
	retrieveDetail : function(oController, oDetailData) {
		...
		oModel.read("/CfmSpecialWorkListSet", {
			async : false,
			filters : [
				new sap.ui.model.Filter('Appno', sap.ui.model.FilterOperator.EQ, oController._vAppno),
				new sap.ui.model.Filter('Prcty', sap.ui.model.FilterOperator.EQ, 'H')
			...
			success : function(data, res) {
				if(data.results && data.results.length) {
					...
					oDetailData.Data.Pernr = oDetailData.Data.Appernr;
				}
			},
			error : function(Res) {
	...
	retrieveDetailTable : function(oController, oDetailTableData, vZappStatAl) {
		...
		oModel.read("/CfmSpecialWorkDetailSet", {
			async : false,
			filters : [
				new sap.ui.model.Filter('Appno', sap.ui.model.FilterOperator.EQ, oController._vAppno),
			...
			success : function(data, res) {
				if(data && data.results.length) {
					...
					data.results.forEach(function(element) {
						...
						element.Outimt = (element.Outimt) ? element.Outimt.replace(/(\d{2})(\d{2})/g, '$1:$2') : "";
						element.Otim11 = (!element.Otim11 || element.Otim11 == "0.00") ? "0" : element.Otim11;
						element.Otim12 = (!element.Otim12 || element.Otim12 == "0.00") ? "0" : element.Otim12;
						element.Otim13 = (!element.Otim13 || element.Otim13 == "0.00") ? "0" : element.Otim13;
						element.Otim14 = (!element.Otim14 || element.Otim14 == "0.00") ? "0" : element.Otim14;
						element.Tim11 = (!element.Tim11 || element.Tim11 == "0.00") ? "0" : element.Tim11;
						element.Tim12 = (!element.Tim12 || element.Tim12 == "0.00") ? "0" : element.Tim12;
						element.Tim13 = (!element.Tim13 || element.Tim13 == "0.00") ? "0" : element.Tim13;
						element.Tim14 = (!element.Tim14 || element.Tim14 == "0.00") ? "0" : element.Tim14;
						element.Tim21 = (!element.Tim21 || element.Tim21 == "0.00") ? "0" : element.Tim21;
						element.Tim22 = (!element.Tim22 || element.Tim22 == "0.00") ? "0" : element.Tim22;
						element.Wtm01 = (!element.Wtm01 || element.Wtm01 == "0.00") ? "0" : element.Wtm01;
						element.Wtm02 = (!element.Wtm02 || element.Wtm02 == "0.00") ? "0" : element.Wtm02;
						element.Wtm03 = (!element.Wtm03 || element.Wtm03 == "0.00") ? "0" : element.Wtm03;
	...
	onSave : function(oController , vPrcty){ // 처리 구분
		...
		var onProcess = function() {
			...
			oModel.create("/CfmSpecialWorkListSet", vOData, {
				success: function(data,res) {
					if(data) {
					} 
				},
...
//				control.ZNKBusyAccessor.onBusy("S",oController);
	...
	onPreApprovalDialog : function(oController) {
		...
		oController._PreApprovalDialog = sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.PreApprovalDialog", oController);
	...
	onValidationData : function(oController, vPrcty){
		...
		if(vPrcty == "C") {
			var isBeguztInputValid = true,
				isEnduztInputValid = true;
			
			vDetailData.some(function(elem, index) {
				if(!elem.Beguzt) {
					isBeguztInputValid = false;
					return true;
				}
				if(!elem.Enduzt) {
					isEnduztInputValid = false;
					return true;
				}
			});
			
			if(!isBeguztInputValid) {
				new control.ZNK_SapBusy.oErrorMessage(oBundleText.getText("LABEL_0728"));	// 728:특근시작시간이 입력되지 않았습니다.
				return "";
			}
			if(!isEnduztInputValid) {
		...
		try {
			rData = common.Common.copyByMetadata(oModel, "CfmSpecialWorkList", vData);
			
			var vDetailDataList = [], tempData = {};
			
			vDetailData.forEach(function(element) {
				tempData = common.Common.copyByMetadata(oModel, "CfmSpecialWorkDetail", element);
				
				tempData.Datum = "\/Date("+ common.Common.getTime(tempData.Datum)+")\/";
				tempData.Beguzt = (tempData.Beguzt) ? tempData.Beguzt.replace(/[^\d]/g, '') : undefined;
				tempData.Enduzt = (tempData.Enduzt) ? tempData.Enduzt.replace(/[^\d]/g, '') : undefined;
				tempData.Outimt = (tempData.Outimt) ? tempData.Outimt.replace(/[^\d]/g, '') : undefined;
				
				delete tempData.Beguz;
				delete tempData.Enduz;
				delete tempData.Outim;
				
				vDetailDataList.push(tempData);
			});
			
			rData.CfmSpecialWorkNav = vDetailDataList;
		} catch(ex) {
	...
	onDelete : function(oController , vPrcty){ // 처리 구분
		...
		var onProcess = function(){
				...
				oModel.remove("/CfmSpecialWorkListSet(Appno='" + vDetailData.Appno + "')", {
					success: function(data,res) {
					},
					error: function(Res) {
