jQuery.sap.declare("ZUI5_HR_VacationMemberUse.control.OrgSearch");
...
ZUI5_HR_VacationMemberUse.control.OrgSearch = {
	/** 
	* @memberOf ZUI5_HR_VacationMemberUse.control.OrgSearch
	*/	
...
//		}
		...
		if(_gAuth == "M") oFilters.push(new sap.ui.model.Filter("ReqAuth", sap.ui.model.FilterOperator.EQ, "2"));
...
//			oTable.bindItems("/OrgListSet");
	...
	onClose : function(oEvent) {
		...
		if(oControl){
			oControl.setValue("");
			oController._ListCondJSonModel.setProperty("/Data/Orgeh", undefined); 
			oController._ListCondJSonModel.setProperty("/Data/Orgtx", undefined); 
		}
	...
	onKeyUp : function(oEvent) {
		if(oEvent.which == 13) {
			ZUI5_HR_VacationMemberUse.control.OrgSearch.OrgSearch();
		}
	},
	...
	onConfirm : function(oEvent) {
		...
		if(oInput) {
			if(vSelectedOrg && vSelectedOrg.length) {
				...
				oController._ListCondJSonModel.setProperty("/Data/Orgeh", vSelectedOrg[0].Orgeh); 
				oController._ListCondJSonModel.setProperty("/Data/Orgtx", vSelectedOrg[0].Stext); 
			}
		}
	...
	onClick : function(oEvent) {
		...
		ZUI5_HR_VacationMemberUse.control.OrgSearch.clicks = ZUI5_HR_VacationMemberUse.control.OrgSearch.clicks + 1;
		
		if(ZUI5_HR_VacationMemberUse.control.OrgSearch.clicks == 1) {
			setTimeout( ZUI5_HR_VacationMemberUse.control.OrgSearch.clearClicks, 500);
		} else if(ZUI5_HR_VacationMemberUse.control.OrgSearch.clicks == 2) {
			ZUI5_HR_VacationMemberUse.control.OrgSearch._vSPath = vContext.sPath;
		}
	},
	...
	clearClicks : function () {
		var oView = sap.ui.getCore().byId("ZUI5_HR_VacationMemberUse.ZUI5_HR_VacationMemberUseList");
		var oController = oView.getController();
		ZUI5_HR_VacationMemberUse.control.OrgSearch.clicks = 0;
		ZUI5_HR_VacationMemberUse.control.OrgSearch._vSPath = -1;
	},
	...
	onDblClick : function(oEvent){
		...
		var oController = oView.getController();
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_OrgSearch_TABLE");
		if(ZUI5_HR_VacationMemberUse.control.OrgSearch._vSPath == null || ZUI5_HR_VacationMemberUse.control.OrgSearch._vSPath == -1) return ;
		var vIndex = ZUI5_HR_VacationMemberUse.control.OrgSearch._vSPath.replace("/OrgListSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		ZUI5_HR_VacationMemberUse.control.OrgSearch.onConfirm();
	},
