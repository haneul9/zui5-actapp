...
sap.ui.controller("ZUI5_HR_HousingLoan.ZUI5_HR_HousingLoanList", {
	/**
	...
	PAGEID : "ZUI5_HR_HousingLoanList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_HousingLoan.ZUI5_HR_HousingLoanList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_HousingLoan.ZUI5_HR_HousingLoanDetail",
		      data : { Appno : _Appno }
		});
	},
				...
				success: function(data,res){
					if(data && data.results.length){
						for(var i=0;i<data.results.length;i++){
							...
							OneData.Lnhym = (OneData.Lnhym).replace(/(\d{4})(\d{2})/g, '$1.$2');
...
//			}
	...
	onPrintLoan : function(oEvent) {
		...
		var PrintProcess = function(fVal){
			if(fVal && fVal == sap.m.MessageBox.Action.YES) {
				oController.BusyDialog.open();
				setTimeout(onPrintProcess, 100);
			}
		};
		...
		sap.m.MessageBox.show(vInfoTxt, {
			...
			onClose : PrintProcess
		});
	},
	
	onPrintLoan2 : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_HousingLoan.ZUI5_HR_HousingLoanList");
		var oController = oView.getController();
		var vAppno = oEvent.getSource().getCustomData()[0].getValue();
		var vErrorMessage = "";
			...
			oModel.read("/HousingFundLoanSet", {
				...
				success : function(data, res){
					if(data.results && data.results.length) {
						if(data.results[0].Url != "" ) window.open(data.results[0].Url); 
					} 
				},
				error : function (oError) {
			    	var Err = {};
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0103") + "-" + dateFormat.format(curDate) + ".xlsx"	// 103:주택자금 신청
			};
