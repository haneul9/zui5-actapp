				...
				function(data,res){
					if(data && data.results.length){
						for(var i = 0; i <data.results.length; i++ ){
							...
							OneData.ZappStatAl = oController._DetailJSonModel.getProperty("/Data/ZappStatAl");
							if(OneData.Zfilesize / 1024 > 1){
								if(OneData.Zfilesize / 1024 / 1024 > 1){
									OneData.Zfilesize = (OneData.Zfilesize / 1024 / 1024).toFixed(2) + " MB";
								}else{
									OneData.Zfilesize = (OneData.Zfilesize / 1024 ).toFixed(2) + " KB";
								}
							}else{
								OneData.Zfilesize = (OneData.Zfilesize / 1024 ).toFixed(2) + " 바이트";
							}
							OneData.Aedtm = dateFormat.format(new Date(common.Common.setTime(OneData.Aedtm)));
	...
	onFileChange : function(oEvent) {
		...
		if(files) {
			...
			for(var i=0; i<files.length; i++) {
				...
				files[i].Zfiletype = files[i].type;
				if(files[i].size / 1024 > 1){
					if(files[i].size / 1024 / 1024 > 1){
						files[i].Zfilesize = (files[i].size / 1024 / 1024).toFixed(2) + " MB";
					}else{
						files[i].Zfilesize = (files[i].size / 1024 ).toFixed(2) + " KB";
					}
				}else{
					files[i].Zfilesize = (files[i].size / 1024 ) + " 바이트";
				}
				files[i].Aedtm = dateFormat.format(new Date());
				files[i].Ename =  vEname;
				files[i].ZappStatAl = "";
			...
			deleteProcess = function(fVal){
				if(fVal && fVal == sap.m.MessageBox.Action.YES) {
					try {
						...
						for(var i=vContexts.length-1; i>=0; i--) {
...
//						vExistDataFlag
						...
						sap.m.MessageBox.alert(oBundleText.getText("LABEL_2627"));	// 2627:파일 삭제가 완료되었습니다
					} catch(ex) {
	 ...
	 * 첨부된 파일을 삭제처리
	 */
	onDeleteAttachFile2 : function(oEvent) {
		var oController = common.AttachFileAction.oController;
		...
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_CAF_Table");
		var oJSonModel =oTable.getModel();
		var vContexts = oJSonModel.getProperty(oEvent.getSource().getBindingContext().sPath);
		var vIndex = oEvent.getSource().getBindingContext().sPath.replace("/Data/","");
		...
		if(vContexts) {
			deleteProcess = function(fVal){
				if(fVal && fVal == sap.m.MessageBox.Action.YES) {
					try {
						oController.BusyDialog.open();
						if(!vContexts.Appno || vContexts.Appno == ""){
//							var vIndex = parseInt(oEvent.getSource().getBindingContext().sPath.replace("/Data/",""));
							oJSonModel.getProperty("/Data").splice(vIndex, 1);
						}else{
							oModel.remove(
									"/FileListSet(Appno='" + vContexts.Appno 
									+ "',Zworktyp='" + vContexts.Zworktyp
									+ "',Zfilekey='" +vContexts.Zfilekey
									+ "',Zfileseq=" + vContexts.Zfileseq
									+ ")",
									null,
								    function (oData, response) {
								    },
