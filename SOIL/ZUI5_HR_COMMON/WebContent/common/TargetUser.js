jQuery.sap.declare("common.TargetUser");
	...
	onSetTarget : function(oController){
		...
		if(oController._vAppno == ""){ // 신규신청
			if(vEmpLoginInfo.length > 0){
				var OneData = {};
				var vPernr = "", vEncid = "", vEname = "", vPersa = "";
				// 권한
				OneData.Auth = _gAuth;
//				if(_gAuth == "E"){
				if(gReqAuth == "1" || _gAuth == "E"){
					...
					OneData.Persg = vEmpLoginInfo[0].Persg;
					OneData.Persk = vEmpLoginInfo[0].Persk;
					OneData.Encid = vEmpLoginInfo[0].Encid;
				}				
				...
				if(oController._DetailJSonModel){
					...
					oController._DetailJSonModel.setProperty("/Data/Encid",vEncid);
					...
					common.TargetUser.checkOfficeCheck(oController);
				}
			}else{
			...
			oModel.read(oPath, null, null, false,
					function(data, res){
						...
						vTargetData.Data.Persg = data.results[0].Persg;
						vTargetData.Data.Persk = data.results[0].Persk;
						vTargetData.Data.Encid = data.results[0].Encid;
...
//		}
	...
	EmpSearchByTx :function(oEvent){
		...
		if(!vEname || vEname == ""){
			if(oController._TargetJSonModel.getProperty("/Data")){
				...
				vTargetData.Data.Persk = "";
				vTargetData.Data.Persg = "";
				vTargetData.Data.Encid = "";
			...
			if(vEmpSearchResult.EmpSearchResultSet.length == 1){
				...
				vTargetData.Data.Entda    = vContext.Entda == null ? null : dateFormat2.format(new Date(common.Common.setTime(vContext.Entda))); // 입사일
				vTargetData.Data.Famdt    = vContext.Famdt == null ? null : dateFormat.format(new Date(common.Common.setTime(vContext.Famdt))); // 결혼기념일
				vTargetData.Data.Auth     = oController._DetailJSonModel.getProperty("/Data/Auth");
				vTargetData.Data.ZappStatAl     = oController._DetailJSonModel.getProperty("/Data/ZappStatAl");
				vTargetData.Data.Persk = vContext.Persk;
				vTargetData.Data.Persg = vContext.Persg;
				vTargetData.Data.Encid = vContext.Encid;
				...
				oController._DetailJSonModel.setProperty("/Data/Encid",vContext.Encid);
	...
	onESSelectPerson : function(oEvent) {
		...
		if(vEmpSearchResult && vEmpSearchResult.length > 0 ) {
			...
			if(oController._MandateDialog && oController._MandateDialog.isOpen()){
				...
				vTargetData.Data.Persk    = mEmpSearchResult.getProperty(vContext + "/Persk");
				vTargetData.Data.Persg    = mEmpSearchResult.getProperty(vContext + "/Persg");
				vTargetData.Data.Auth     = oController._DetailJSonModel.getProperty("/Data/Auth");
				vTargetData.Data.ZappStatAl     = oController._DetailJSonModel.getProperty("/Data/ZappStatAl");
				vTargetData.Data.Encid    = mEmpSearchResult.getProperty(vContext + "/Encid");
				// 상세 내역에 대상자 정보 추가
				 ...
				 oController._DetailJSonModel.setProperty("/Data/Encid",mEmpSearchResult.getProperty(vContext + "/Encid"));
				 oController._TargetJSonModel.setData(vTargetData);
	...
	onDblClick : function(oEvent){
		var oController = common.TargetUser.oController;
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_EmpSearchResult_Table");
		if(common.TargetUser._vSPath == null || common.TargetUser._vSPath == -1) return ;
		var vIndex = common.TargetUser._vSPath.replace("/EmpSearchResultSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		common.TargetUser.onESSelectPerson();
	},
