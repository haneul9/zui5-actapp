jQuery.sap.declare("common.Password");
...
common.Password = {
	/** 
	* @memberOf common.Password
	*/	
	
	oController : null,
	_PageKey : "",
	oModel : new sap.ui.model.odata.ODataModel("/sap/opu/odata/sap/ZHR_PAYSLIP_SRV/", true, undefined, undefined, { "Cache-Control": "max-age=0"}, undefined, undefined, true),
	
	
	openPasswordDialog : function(oController, _PageKey){
		if(common.Common.checkNull(_PageKey)){
			sap.m.MessageBox.error("권한이 없어서 해당페이지에 접근할 수 없습니다.\n 관리자에게 문의바랍니다.");
			return;
		}
		if(!oController._Password){
			oController._Password = sap.ui.jsfragment("fragment.Password", oController);
		}
		sap.ui.getCore().byId(oController.PAGEID + "_Secpw").setValue("");
		oController._Password.open();
		common.Password._PageKey = _PageKey;
	},
	
	onCheckPassword  : function(oController, oEvent){
		var pw = sap.ui.getCore().byId(oController.PAGEID + "_Secpw").getValue();
		
		var createData = { Prcty : "C" , Secpw :  common.Password.SHA256(pw) };
		
		var oModel = common.Password.oModel;
		oModel.create("/PaySlipPasswordSet", createData, {
			success : function(data, res) {
				oController.goToPage(oController, common.Password._PageKey, "");
    ...
    openPasswordChange : function(oController, oEvent){
		if(!oController._PasswordChange){
			oController._PasswordChange = sap.ui.jsfragment("fragment.PasswordChange", oController);
		}
	...
	onChangePassword  : function(oController, oEvent){
		...
		var createData = { Prcty : "S" , SecpwOrg :  common.Password.SHA256(pw), SecpwChg1 :  common.Password.SHA256(pw2), SecpwChg2 :  common.Password.SHA256(pw3) };
