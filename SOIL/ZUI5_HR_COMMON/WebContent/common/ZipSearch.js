...
common.ZipSearch = {
	...
	*/	
	...
	getAddr : function(vIndex) {	
		...
		if(typeof vIndex == "object") vIndex = 1 ;
		...
		var onProcess = function(){
			...
			$.ajax({
						...
						if(jsonStr != null){
							...
							if(vIndex && vIndex != "" ){
								...
								common.ZipSearch._vTotalResult = jsonStr.results.common.totalCount;
								 // 도로명주소 검색 결과 (1180건)
						        oResult.setText("ㆍ " + oBundleText.getText("LABEL_2840") + " (" + common.ZipSearch._vTotalResult + oBundleText.getText("LABEL_0001") + ")" );	// 0001:건, 2840:도로명주소 검색 결과
			    ...
			    ,error: function(xhr,status, error){
			    	...
			    	sap.m.MessageBox.error(oBundleText.getText("LABEL_2841"));	// 2841:주소 검색 중 에러발생
			    }
			});
			 oController.BusyDialog.close();
		}
	...
	checkSearchedWord: function(obj){
		if(obj.length >0){
			...
			if(expText.test(obj) == true){
				alert(oBundleText.getText("LABEL_2842")) ;	// 2842:특수문자를 입력 할수 없습니다.
			...
			for(var i=0; i<sqlArray.length; i++){
				...
				if (regex.test(obj) ) {
				    sap.m.MessageBox.error("\"" + sqlArray[i]+"\"" + oBundleText.getText("LABEL_2843"));	// 2843:와(과) 같은 특정문자로 검색할 수 없습니다.
				    obj =obj.replace(regex, "");
					return false;
				}
			}
		}else{
			sap.m.MessageBox.error(oBundleText.getText("LABEL_1037"));	// 1037:검색어를 입력하여 주세요.
			return false;
		}
	...
	onSelectAddress : function(oEvent){
		...
		if(!vContexts || vContexts.length == 0){
			sap.m.MessageBox.alert(oBundleText.getText("LABEL_2844"));	// 2844:주소를 선택하여 주세요.
			return;
		}
