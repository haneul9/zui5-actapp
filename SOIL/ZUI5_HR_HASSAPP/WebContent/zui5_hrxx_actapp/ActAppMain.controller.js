...
//		this.onPressSearch();
	...
	setTable: function(oController){
		var oTable = $("#"+oController.PAGEID + "_Table").bootstrapTable({
...
//		    bDeferRender : true,
	        columns: [
	                  	{ title: 'Item ID', field: "Pbtxtc"},
			            { title: 'Item ID2', field: "Count" },
			             { title: 'Item ID3', field: "BalErrCnt"},
			            { title: 'Item ID4', field: "AccRate" },
			            { title: 'Item ID5', field: "Lat"},
			            { title: 'Item ID6', field: "Lng"}
			        ]
	    });
	...
	onPressSearch : function(oEvent) {
		...
		if(!oController.BusyDialog) {
			...
			oController.BusyDialog.addContent(new sap.m.BusyIndicator({text : "검색중입니다. 잠시만 기다려 주십시오."}));
