...
sap.ui.controller("zui5_hrxx_actapp.ActAppUpload", {
	...
	_BasicControl : [{label : "No.", id : "Recno", Align : "Center", Width : 40, control : "Text", required : false},
	                 ...
	                 {label : "사번", id : "Pernr", Align : "Left", Width : 100, control : "Text", required : true},
	                 {label : "성명", id : "Ename", Align : "Left", Width : 100, control : "Text", required : false},
	                 {label : "소속", id : "Per_orgeh_Tx", Align : "Left", Width : 150, control : "Text", required : false},
...
*/
	...
	onResizeWindow : function(oEvent, oEventId, Data) {
		$("#" + this.PAGEID + "_oIBSheetLayout").css("height", window.innerHeight - 400);
		$("#" + this.PAGEID + "_IBSHEET1").css("height", (window.innerHeight - 400));
	},
	...
	loadActionTypeList : function() {
							...
							if(oMassn) {
								...
								for(var j=0; j<oData.results.length; j++) {
									if(oData.results[j].Massn == "10" || oData.results[j].Massn.substring(0,1) == "8") {
										...
										if(oController._vDocty == "10" && oData.results[j].Massn == "11") {
		...
		if(oController._vDocty == "20") {
			...
			oModel.read("/ActionReasonListSet"  + filterString + "%20and%20Massn%20eq%20%2711%27", 
					null, 
					null, 
					false, 
					function(oData, oResponse) {					
...
//	}
	...
	navToBack : function(oEvent){
		var oView = sap.ui.getCore().byId("zui5_hrxx_actapp.ActAppUpload");
		...
		for(var i=0; i<oController._BasicControl.length; i++) {
			...
			if(i < 2) {
				...
				if(oController._BasicControl[i].id == "Actda") {
					oneCol.Header = "발령대상자|* " + oController._BasicControl[i].label;
		...
		if(oController._vActiveControl && oController._vActiveControl.length) {
			for(var i=0; i<oController._vActiveControl.length; i++) {
				...
				if(Fieldtype.substring(0, 1) == "M") {
					vHeader += "* " + oController._vActiveControl[i].Label;
				} else {
					vHeader += oController._vActiveControl[i].Label;
				}
//				vHeader += oController._vActiveControl[i].Label;
...
//				}
				...
				oneCol.Edit = 0;
				oneCol.Width = 100;
				if(Fieldname.toUpperCase().indexOf("ORGEH") != -1 || Fieldname.toUpperCase().indexOf("STELL") != -1 || Fieldname.toUpperCase().indexOf("ZZEMPWP") != -1) {
	...
	validData : function() {
		...
		var vilidProcess = function() {	
			...
			UploadDataSheet.SetCellValue(idx + vHeaderCount, "Upbigo", "데이터를 검증 중입니다. 잠시만 기다려 주십시오.", 0);
...
//				var vDate = new Date();
//				vDate.setUTCFullYear(parseInt(createData.Actda.substring(0,4)));
//				vDate.setUTCMonth(parseInt(createData.Actda.substring(4,6)) - 1);
//				vDate.setUTCDate(parseInt(createData.Actda.substring(6,8)));
			...
			if(oController._vActiveControl && oController._vActiveControl.length) {
				for(var j=0; j<oController._vActiveControl.length; j++) {
					...
					if(Fieldtype == "M4" || Fieldtype == "O4") {
						...
						if(dVal != null && dVal != "") {
//							var vDate = new Date();
//							vDate.setUTCFullYear(parseInt(dVal.substring(0,4)));
//							vDate.setUTCMonth(parseInt(dVal.substring(4,6)) - 1);
//							vDate.setUTCDate(parseInt(dVal.substring(6,8)));
		...
		if(!oController.BusyDialog) {			
			oController.BusyDialog = new sap.m.Dialog({showHeader : false}); 
			oController.BusyDialog.addContent(new sap.m.BusyIndicator({text : "데이터 검증 중입니다. 잠시만 기다려 주십시오."}));
			oController.getView().addDependent(oController.BusyDialog);
		} else {
			oController.BusyDialog.removeAllContent();
			oController.BusyDialog.destroyContent();
