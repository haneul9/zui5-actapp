...
//				        	   type : "Emphasized",
		...
		var oSyncToolbar = new sap.m.Toolbar(oController.PAGEID + "_SyncToolbar", {
			...
			content : [new sap.m.ToolbarSpacer(),
			           new sap.m.Button(oController.PAGEID + "_Sync_Rec_Btn",{text: "재입사 정보 재조회", icon : "sap-icon://synchronize", press : oController.syncRecPerson, visible : false, type : "Default"})]
		}).addStyleClass("L2PToolbarNoBottomLine");
		...
		var oListPanel = new sap.m.Panel({
			...
			headerToolbar : new sap.m.Toolbar({
				...
				content : [new sap.ui.core.Icon({ src: "sap-icon://open-command-field", size : "1.0rem"}),
				           new sap.m.Label({text : "발령대상자", design : "Bold"}).addStyleClass("L2PFontFamilyBold"),
