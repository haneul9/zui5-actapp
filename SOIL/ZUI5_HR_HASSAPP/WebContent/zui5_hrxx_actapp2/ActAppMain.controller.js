					...
					function(oData, oResponse) {
						if(oData && oData.results) {
							for(var i=0; i<oData.results.length; i++) {								
								...
								var vCheck = oData.results[i].Actda.toString().substring(0,1);
								var vActda = "";
								if(vCheck == "/"){
									var tmp = oData.results[i].Actda.split("(")[1];
										tmp = tmp.split(")")[0] * 1;
										vActda = new Date(tmp);	
								}else {
									vActda = new Date(oData.results[i].Actda);
								}
								oneData.Actda = new Date(common.Common.setTime(vActda));
								
								
								var vCheck = oData.results[i].Reqda.toString().substring(0,1);
								var vReqda = "";
								if(vCheck == "/"){
									var tmp = oData.results[i].Reqda.split("(")[1];
										tmp = tmp.split(")")[0] * 1;
										vReqda = new Date(tmp);
								}else{
									vReqda = new Date(oData.results[i].Reqda);
								}
								oneData.Reqda = new Date(common.Common.setTime(vReqda));
								

								var vCheck = oData.results[i].Datlo.toString().substring(0,1);
								if(vCheck == "/"){
									var tmp = oData.results[i].Datlo.split("(")[1];
										tmp = tmp.split(")")[0] * 1;
									var date = new Date(tmp);
									
									oneData.Datlo = new Date(date);
								}
