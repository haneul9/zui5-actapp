...
//		common.GoogleMap.setMap(vAfterData.Gps, vAfterData.Zzempwp_Tx, "", "");
		...
		for(var i=0; i<vDisplayControl.length; i++) {
			...
			if (i % 2 == 0)	oRow = new sap.ui.commons.layout.MatrixLayoutRow({height:"40px"});
	...
	setSubjectListColumn : function(oController) {
		...
		var vColumns = [ {id : "Ename", label : "성명", control : "ro", width : "170", align : "center"},
		                 {id : "Acttx", label : "발령유형", control : "txt", width : "250", align : "left"},
		...
		if(oController._vStatu != "00") {
			for(var i=0; i<oController.vDisplayControl.length; i++) {
				...
				oController.oSubjectList.insertColumn(c_idx, vHeaderText,'ro', "150", 'na','center','middle');
				c_idx++;
		...
		if( vTTableWidth >  vTColumnWidth ){
			oController.oSubjectList.insertColumn(c_idx, "",'ro', vTTableWidth - vTColumnWidth, 'na','center','middle');
			c_idx++;
		}
					...
					if(oData.results && oData.results.length) {
						...
						for(var i=0; i<oData.results.length; i++) {
							...
							if(oData.results[i].Batyp == "A") {
								...
								//수정완료
								oneData.Batyp = "After";
								oneData.Voltid = oData.results[i].VoltId;
...
//								oneData.B_Batyp = oData.results[i].Batyp;
							...
							var vEname_Html = "";
							if(oData.results[i].Perid){
								if(icon_src == ""){
									vEname_Html = "<table cellpadding=0 cellspacing=0 border=0 style='background-color:transparent'>"
				                        + "<tr>"
				                        + "<td style='padding-left:5px; border:0px;'>"
				                        + "<div id='" + oController.PAGEID + "_row_name_" + (i+1) + "' class='L2PFontFamily' style='color:blue; cursor:pointer;' onclick='onInfoViewPopup3(" + (i+1) + ")'>" + oData.results[i].Ename +" (" + oData.results[i].Perid + ")</div>";
...
//				                        + "<div class='L2PFontFamily'>(" + oData.results[i].Pernr + ")</div>";   
								...
								if(icon_src == ""){
									vEname_Html = "<table cellpadding=0 cellspacing=0 border=0 style='background-color:transparent'>"
				                        + "<tr>"
				                        + "<td style='padding-left:5px; border:0px;'>"
				                        + "<div id='" + oController.PAGEID + "_row_name_" + (i+1) + "' class='L2PFontFamily' style='color:blue; cursor:pointer;' onclick='onInfoViewPopup3(" + (i+1) + ")'>" + oData.results[i].Ename +" (" + oData.results[i].Pernr + ")</div>";
...
//				                        + "<div class='L2PFontFamily'>(" + oData.results[i].Pernr + ")</div>";   
							...
							if(oData.results[i].Batyp == "A") SubjectListOneData.data.push("<span style='font-size:0.875rem; font-weight:bold; color:blue'>After</span>");
							else SubjectListOneData.data.push("<span style='font-size:0.875rem; font-weight:bold; color:black'>Before</span>");
							...
							if(oController.oSubjectList) {
								...
								oController.oSubjectList.setCellTextStyle((i+1), 2, "padding-left : 5px;");
							}
						}
					}
				},
	...
	setRecSubjectListColumn : function(oController) {
		...
		var icon2 = "<img src='/sap/bc/ui5_ui5/sap/ZUI5_HR_HCOMMON/images/Error.jpg' style='width :15px; height : 15px'>";
		var icon3 = "<img src='/sap/bc/ui5_ui5/sap/ZUI5_HR_HCOMMON/images/Lock.jpg' style='width :15px; height : 15px'>";
		var icon4 = "<img src='/sap/bc/ui5_ui5/sap/ZUI5_HR_HCOMMON/images/check-icon.png' style='width :15px; height : 15px'>";
		...
		var vColumns = [ {id : "Ename", label : "성명", control : "ro", width : "150", align : "center"},
		                 {id : "Acttx", label : "발령유형", control : "ro", width : "250", align : "left"},
		                 {id : "Actda1", label : "발령일", control : "ro", width : "100", align : "center"},
		               ];
		...
		if(oController._vStatu != "00") {
			for(var i=0; i<oController.vDisplayControl.length; i++) {
				...
				if(oController.PAGEID == "ActAppDocumentView"){
					oController.oSubjectList.insertColumn(c_idx, vHeaderText,'ro', "150", 'na','center','middle'); 
				} else {
					oController.oSubjectList.insertColumn(c_idx, vHeaderText,'ro', "150", 'na','center','middle'); 
				}
		...
		for(var i = 0 ; i < c_idx ; i++){
			if(i == 0)	vResizeTx += "false";
			else vResizeTx += ",false";
		}
		
		oController.oSubjectList.enableResizing(vResizeTx);
...
//				                        + "<div class='L2PFontFamily'>(" + oData.results[i].Pernr + ")</div>";   
	...
	onAfterRenderingTable : function(oController) {
		...
		if(oController.PAGEID == "ActAppDocument")
			$("#" + oController.PAGEID + "_SubjectList").css("height", window.innerHeight - 340);
		else if(oController.PAGEID == "ActRecDocument"){
			$("#" + oController.PAGEID + "_SubjectList").css("height", window.innerHeight - 310);
		}else if(oController.PAGEID == "ActAppComplete"){
			$("#" + oController.PAGEID + "_SubjectList").css("height", window.innerHeight - 110);
		}else if(oController.PAGEID == "ActAppDocumentView"){
		...
		oController.oSubjectList.setStyle(
				"min-height : 35px !important; font-size : 0.875rem;   border-left : 1px solid #e9e9e9; border-bottom : 1px solid #e9e9e9; vertical-align : middle; background-color : #d9d9d9;",
				"height : 35px !important; font-size : 0.875rem;  border-left : 1px solid #e9e9e9; border-bottom : 1px solid #e9e9e9; vertical-align : middle; background-color:rgb(255,255,255);",
				"", "");
		oController.oSubjectList.attachEvent("onCheckbox", oController.doOnCheck);
		oController.oSubjectList.init();
//		oController.oSubjectList.enableColumnAutoSize(true);
