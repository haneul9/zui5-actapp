...
//	    	if(vScryn != "X") {
////	    		document.location.href = "/sap/bc/ui5_ui5/sap/ZUI5_HR_HCOMMON/NoAuth.html?menuid=" + vLpmid;
//	    		return;
//	    	}
		} catch(ex) {
			common.Common.log(ex);
...
//		}		
		...
		try {
			oModel.read("/ActionAppReqSortSet", 
					...
					function(oData, oResponse) {					
						if(oData && oData.results.length) {
							vActionAppReqSort.ActionAppReqSortSet.push({Srtfc : "0000", Srtft : "-- 선택 --"});
							for(var i=0; i<oData.results.length; i++) {
		...
		try {
			oModel.read("/ActionAppReqSortTypeSet", 
					...
					function(oData, oResponse) {					
						if(oData && oData.results.length) {
							vActionAppReqSortType.ActionAppReqSortTypeSet.push({Srttc : "0000", Srttt : "-- 선택 --"});
							for(var i=0; i<oData.results.length; i++) {
		...
		//문서유형 리스트
		var vDocTypeList = {DocTypeListSet :[{Docty : "10", Doctx : "일반 발령", PageId : "zui5_hrxx_actapp2.ActAppDocument"},
		                                      {Docty : "20", Doctx : "채용 발령(파견직 포함)", PageId : "zui5_hrxx_actapp2.ActRecDocument"},
		                                      {Docty : "30", Doctx : "조직 개편", PageId : "zui5_hrxx_actapp2.ActOrgDocument"}
		                                     ]};
		...
		// 국가 코드 리스트
 		...
 		oCommonModel.read("/EmpCodeListSet?$filter=Field eq 'Natio' and PersaNc eq 'X'", 
 					...
 					function(oData, oResponse) {
 						if(oData && oData.results.length) {
 							vCode.natioCode.push({Field : "Natio", Ecode : "", Etext : "값 없음"});
