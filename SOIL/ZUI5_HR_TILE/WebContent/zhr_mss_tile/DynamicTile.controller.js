...
(function(){
	...
	sap.ui.controller("zhr_mss_tile.DynamicTile",{
//	sap.ui.controller("sap.ushell.components.tiles.zhr_mss_tile.DynamicTile",{
		timer:null,
		oDataRequest:null,
...
//			c=sap.ushell.components.tiles.utilsRT.getConfiguration(t,true,false),
			...
			// 시맨틱오브젝트 별로 타일 background color 설정
//			var oObject = c.navigation_semantic_object;
//			if(oObject == "ZHR_AGE") v.addStyleClass("backgroundPurple");
//			else if(oObject == "ZHR_ATTDSTATUS" || oObject == "ZHR_VACATION") v.addStyleClass("backgroundGray");
//			else if(oObject == "ZHR_OVEROT" || oObject == "ZHR_OTPAYCHART") v.addStyleClass("backgroundBlue");
...
//			}
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zhr_mss_tile.DynamicTile.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zhr_mss_tile.DynamicTile.controller");
            }
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zhr_mss_tile.DynamicTile.controller"
                );
                // call again later
