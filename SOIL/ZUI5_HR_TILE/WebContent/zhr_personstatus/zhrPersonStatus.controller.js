...
(function(){
	...
	sap.ui.controller("zhr_personstatus.zhrPersonStatus",{
//	sap.ui.controller("sap.ushell.components.tiles.zhr_personstatus.zhrPersonStatus",{
		timer:null,
		oDataRequest:null,
...
//			v.addStyleClass("backgroundPurple");
		...
		onAfterRendering: function() { 
			  ...
			  var readAfterProcess = function() {
				  if(vData.results.Cnt01 == 0 &&
					  ...
					  vData.results.Cnt05 == 0 &&
					  vData.results.Cnt06 == 0){
					  vSummaryData.push({label : "" , value : 1, color : "#f5f5f5", highlight: "#f5f5f5"});
					  
//					  var ctx = document.getElementById("chart-area2").getContext("2d");
//					  ctx.beginPath();
//					  ctx.arc(44, 44, 44, 0, 2 * Math.PI);
//					  ctx.fillStyle = "#ffff00";
//					  ctx.fill();
					  
				  }else{
					  vSummaryData.push({label : "운전직" , value : vData.results.Cnt06, color : "#e62828", highlight: "#e62828"});
					  vSummaryData.push({label : "관리직" , value : vData.results.Cnt01, color : "#2ecc71", highlight: "#2ecc71"});
					  vSummaryData.push({label : "사무기술" , value : vData.results.Cnt02, color : "#ff8000", highlight: "#ff8000"});
					  vSummaryData.push({label : "사무행정" , value : vData.results.Cnt03, color : "#eacd58", highlight: "#eacd58"});
					  vSummaryData.push({label : "기능직" , value : vData.results.Cnt04, color : "#1ab9dd", highlight: "#1ab9dd"});
					  vSummaryData.push({label : "기타직" , value : vData.results.Cnt05, color : "#6d6c6c", highlight: "#6d6c6c"});
//					  vSummaryData.push({label : "특수직" , value : vData.results.Cnt05, color : "#6d6c6c", highlight: "#6d6c6c"});
					  
					  					  
//					  var ctx = document.getElementById("chart-area2").getContext("2d");
//					  window.myPie2 = new Chart(ctx).Pie(vSummaryData);
				  }
			  ...
			  oModel.read("/MssPersonstatusSet",
					null, 
					null, 
					true, 
					function(oData, oResponse) {
						if(oData && oData.results.length) {
							...
							oneData.Cnt01 = parseInt(oData.results[0].Cnt01);
							oneData.Cnt02 = parseInt(oData.results[0].Cnt02);
							oneData.Cnt03 = parseInt(oData.results[0].Cnt03);
							oneData.Cnt04 = parseInt(oData.results[0].Cnt04);
							oneData.Cnt05 = parseInt(oData.results[0].Cnt05);
							oneData.Cnt06 = parseInt(oData.results[0].Cnt06);
							oneData.Prdtx = oData.results[0].Prdtx;
...
//			}
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zhr_personstatus.zhrPersonStatus.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zhr_personstatus.zhrPersonStatus.controller");
            }
        ...
        errorHandlerFn: function (oMessage) {
            ...
            jQuery.sap.log.error(
                "Failed to update data via service "
                    ...
                    + ": " + sMessage,
                null,
                "zhr_personstatus.zhrPersonStatus"
            );
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zhr_personstatus.zhrPersonStatus.controller"
                );
                // call again later
