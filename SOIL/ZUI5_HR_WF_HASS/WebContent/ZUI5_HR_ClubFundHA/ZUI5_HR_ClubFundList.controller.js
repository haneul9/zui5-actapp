...
sap.ui.controller("ZUI5_HR_ClubFundHA.ZUI5_HR_ClubFundList", {
	/**
...
//				oController._vPersa = vEmpLoginInfo[0].Persa ; // 사원검색을 위해 필요한 인사영역 
			...
			var JSonData = {Data : {Apbeg : dateFormat.format(prevDate), Apend : dateFormat.format(curDate), Auth : _gAuth}};
...
//		oController.onInitControl(oController);
	...
	onAfterShow : function(oEvent) {
		...
		if(sap.ui.getCore().byId(this.PAGEID + "_Zclub").getSelectedItem()) this.onPressSearch();		
	},
...
//		if(vData != undefined) oController.onGetCountDown(oController ,vData.Pernr );
	...
	onSelectRow : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_ClubFundHA.ZUI5_HR_ClubFundList");
		...
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_ClubFundHA.ZUI5_HR_ClubFundDetail",
	...
	setZclub : function(oController, vPernr){
		...
		if(oZclub.getItems().length == 0){
			...
			oModel.read("/ClubGroupListSet?$filter=Appty eq '2'", null, null, false,
					function(data, res){
						if(data && data.results.length){
							
							if(data.results[0].Appty == "X"){
							...
							if(_gAuth != "H"){
								for(var i=0; i<data.results.length; i++){
									if(data.results[i].Mangnr == vPernr || data.results[i].Chairnr == vPernr || data.results[i].Stfnr == vPernr){
										...
										if(!oZclub.getSelectedItem()) oZclub.setSelectedKey(data.results[i].Zclub);
									}
								}
							} else {
								for(var i=0; i<data.results.length; i++){
	...
	onPressSearch : function(oEvent){
		...
		if(!SerchCond.Zclub) {
			sap.m.MessageBox.alert("인포멀그룹을 선택하여 주십시오.", {title : "오류"});
			return;
		}	
		...
		function Search(){
			var oPath = "/ClubSupportFundHeadSet";
			var filterString = "";
			...
			if(SerchCond.Zclub != ""){
				if(filterString == ""){
					filterString = "/?$filter=Zclubr eq '" + SerchCond.Zclub + "'";	
				} else {
