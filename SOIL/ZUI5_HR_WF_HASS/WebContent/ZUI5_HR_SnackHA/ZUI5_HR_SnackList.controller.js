...
sap.ui.controller("ZUI5_HR_SnackHA.ZUI5_HR_SnackList", {
	/**
...
//		oController.onInitControl(oController);
	...
	onSelectRow : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_SnackHA.ZUI5_HR_SnackList");
		...
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_SnackHA.ZUI5_HR_SnackDetail",
	...
	// 계산근거
	onCalrsn : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_SnackHA.ZUI5_HR_SnackList");
		var oController = oView.getController();
		
		var vData = {Data : {}};
		
		var vCalrsn = oEvent.getSource().getCustomData()[0].getValue();
		
		var oModel = sap.ui.getCore().getModel("ZHR_SNACK_SRV");
		var oPath = "/SnackCalrsnTextSet?$filter=Zfkey eq '" + vCalrsn + "'";
		
		oModel.read(oPath, null, null, false, 
				function(data,res){
					if(data && data.results.length){			
						vData.Data.Ztext = data.results[0].Ztext;
					}
				}, function(Res){
		...
		if(!oController._CalrsnPopover) {
			oController._CalrsnPopover = sap.ui.jsfragment("ZUI5_HR_SnackHA.fragment.DisplayCalrsn", oController);
			oView.addDependent(oController._CalrsnPopover);
		}
		
		var oJSONModel = oController._CalrsnPopover.getModel();
			oJSONModel.setData(vData);
		
		oController._CalrsnPopover.openBy(oEvent.getSource());
	},
	...
	onPopover : function(oEvent){
		...
		if(!oController._ZappResnPopover) {
			oController._ZappResnPopover = sap.ui.jsfragment("ZUI5_HR_SnackHA.fragment.DisplayZappResn", oController);
	...
	onPressSearch : function(oEvent){
		...
		function Search(){
			var oPath = "/SnackExpensesApplSet";
			var filterString = "";
