...
(function(){
	...
	sap.ui.controller("zui5_hrxx_dynamictile.DynamicTile",{
//		sap.ui.controller("sap.ushell.components.tiles.zui5_hrxx_dynamictile.DynamicTile",{
		timer:null,
		oDataRequest:null,
...
//						a.getView().addContent(T);
		...
		onPress:function(){
			...
			T=V.chip;
			if(t == "#ZHR_EDUCATION-display"){
				  var vEnCodePerno = "" ;
				  var oEncodeIDModel = new sap.ui.model.odata.ODataModel("/sap/opu/odata/sap/ZHR_PASSWORD_SRV/", true, undefined, undefined, { "Cache-Control": "max-age=0"}, undefined, undefined, true);
				  var oPath = "/IdEncryptionSet";
				  oEncodeIDModel.read(oPath,null,null,false,function(Data,Res){
						if(Data && Data.results.length){
							vEnCodePerno = Data.results[0].Loginid ;
						}
					},function(Res){
				  });
				
				  if(vEnCodePerno == ""){
					  sap.m.MessageBox.alert("사원번호의 암호화에 실패하였습니다. \n관리자에 문의바랍니다.",{});
					  return ;
				  }else{
					  var sTargetUrl = "http://dream.lsnikko.com/SAPHR/saphrProcess.jsp?uid=" + vEnCodePerno ;
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zhr_dynamic_tile.DynamicTile.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zhr_dynamic_tile.DynamicTile.controller");
            }
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zhr_dynamic_tile.DynamicTile.controller"
                );
                // call again later
