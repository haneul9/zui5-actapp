...
(function(){
	...
	sap.ui.controller("zui5_hrxx_taskmonitoring.TaskMonitoring",{
		timer:null,
		oDataRequest:null,
		...
		onInit:function(){
			...
			v.setModel(m);
			console.log("TaskMonitoringControl");
...
//						a.getView().addContent(T);
		...
		onAfterRendering: function() { 
			var oPath = "/ToDoListSet";
			var oView = this.getView();       
			var oController = this.getView().getController() ;
			
		    var oModel = new sap.ui.model.odata.ODataModel("/sap/opu/odata/sap/ZHR_TILE_SRV/",true, undefined, undefined, undefined, undefined, undefined, true);
	        try {
	    	   oModel.read(oPath, 
						null, 
						null, 
						...
						function(oData, oResponse) {					
							if(oData.results && oData.results.length) {
								var oneData = {results : {}} ;
								oneData.results.Cnt01 = oData.results[0].Cnt01 == "" ? 0 : parseInt(oData.results[0].Cnt01);
								oneData.results.Cnt02 = oData.results[0].Cnt02 == "" ? 0 : parseInt(oData.results[0].Cnt02);
								oneData.results.Cnt03 = oData.results[0].Cnt03 == "" ? 0 : parseInt(oData.results[0].Cnt03);
								
								oneData.results.Period = oData.results[0].Period;
								
								oController._JSonModel.setData(oneData);
...
//				console.log("onPressA");
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zui5_hrxx_taskmonitoring.TaskMonitoring.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zui5_hrxx_taskmonitoring.TaskMonitoring.controller");
            }
        ...
        errorHandlerFn: function (oMessage) {
            ...
            jQuery.sap.log.error(
                "Failed to update data via service "
                    ...
                    + ": " + sMessage,
                null,
                "zui5_hrxx_taskmonitoring.TaskMonitoring"
            );
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zui5_hrxx_taskmonitoring.TaskMonitoring.controller"
                );
                // call again later
