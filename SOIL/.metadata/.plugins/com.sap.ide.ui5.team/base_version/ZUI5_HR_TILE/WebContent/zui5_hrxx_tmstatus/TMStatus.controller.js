...
(function(){
	...
	sap.ui.controller("zui5_hrxx_tmstatus.TMStatus",{
//	sap.ui.controller("sap.ushell.components.tiles.zui5_hrxx_tmstatus.DynamicTile",{
		timer:null,
		oDataRequest:null,
...
//				t.actions.setActionsProvider(function(){return e;});
		...
		onSearch : function(oController, oObject, oAction){
			var vKey = "";
			if(oAction == "approve")		vKey = "H";
			else if(oAction == "manage")	vKey = "M";
...
//			});
			...
			oModel.read("/TimeStatusSet?$filter=Austy eq '" + vKey + "'", null, null, true,
				function(data, Res) {					
					if(data && data.results.length) {
						var oneData = data.results[0];
							oneData.Indicator = "None";
							oneData.Color = parseFloat(oneData.Number) == 0 ? "Neutral" : "Error";
							oneData.Scale = "hr";

							oneData.Label1 = oneData.Cntu1 + "/" + oneData.Cntc1;
							oneData.Label2 = oneData.Cntu2 + "/" + oneData.Cntc2;
							oneData.Label3 = oneData.Cntu3 + "/" + oneData.Cntc3;
							
							oneData.Number = parseFloat(oneData.Number);
							oneData.Cntc1 = parseFloat(oneData.Cntc1.replace(/,/g, ""));
							oneData.Cntc2 = parseFloat(oneData.Cntc2.replace(/,/g, ""));
							oneData.Cntc3 = parseFloat(oneData.Cntc3.replace(/,/g, ""));
							oneData.Cntu1 = parseFloat(oneData.Cntu1.replace(/,/g, ""));
							oneData.Cntu2 = parseFloat(oneData.Cntu2.replace(/,/g, ""));
							oneData.Cntu3 = parseFloat(oneData.Cntu3.replace(/,/g, ""));
							
							
						oController.getView().getModel().setProperty("/data", oneData);
					}
				...
				function(Res) {
					if(Res.response.body){
						...
						oController.getView().getModel().setProperty("/data", null);
					}
				}
			);
		},
...
//			}
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zui5_hrxx_tmstatus.TMStatus.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zui5_hrxx_tmstatus.TMStatus.controller");
            }
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zui5_hrxx_tmstatus.TMStatus.controller"
                );
                // call again later
