...
(function(){
	...
	sap.ui.controller("zui5_hrxx_mss_chart.chart",{
		timer:null,
		oDataRequest:null,
...
//						a.getView().addContent(T);
		...
		onAfterRendering: function() { 
			...
			var oId = oSemanticObject + "-" + oAction;	// MSS : ZHR_APPROVALBOX-manage, HASS : ZHR_APPROVALBOX-approve
...
//		    vSummaryData.push({label : "참조" , value : vData.results.Cnt04, color : "#999999", highlight: "#999999"});
//		    
//		    var ctx = document.getElementById("chart-" + oId).getContext("2d");
//		    window.myPie2 = new Chart(ctx).Pie(vSummaryData);
//		    oController._JSonModel.setData(vData);
//		    
//			oController.getView().getModel().setProperty('/data/Cnt01', vData.results.Cnt01);
//			oController.getView().getModel().setProperty('/data/Cnt02', vData.results.Cnt02);
//			oController.getView().getModel().setProperty('/data/Cnt03', vData.results.Cnt03);
//			oController.getView().getModel().setProperty('/data/Cnt04', vData.results.Cnt04);
//			oController.getView().getModel().setProperty('/data/Cnt05', vData.results.Cnt05);
//			oController.getView().getModel().setProperty('/data/Prdtx', vData.results.Prdtx);
...
//			}
		...
		onUpdateDynamicData:function(){
			...
			if(!n){n=0;}
				...
				jQuery.sap.log.warning("Refresh Interval "+n+" seconds for service URL "+c.service_url+" is less than 10 seconds, which is not supported. "+"Increased to 10 seconds automatically.",null,"zui5_hrxx_mss_chart.chart.controller");
				n=10;
			}
			if(c.service_url){
				this.loadData(n);
        ...
        onSaveRuntimeSettings: function (oSettingsView) {
            ...
            function logErrorAndReject(oError) {
                jQuery.sap.log.error(oError, null, "zui5_hrxx_mss_chart.chart.controller");
            }
        ...
        loadData: function (nservice_refresh_interval) {
            ...
            if (nservice_refresh_interval > 0) {
                ...
                jQuery.sap.log.info(
                    ...
                    null,
                    "zui5_hrxx_mss_chart.chart.controller"
                );
                // call again later
