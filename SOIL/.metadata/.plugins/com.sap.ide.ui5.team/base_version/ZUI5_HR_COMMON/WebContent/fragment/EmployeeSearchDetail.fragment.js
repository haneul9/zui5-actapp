sap.ui.jsfragment("fragment.EmployeeSearchDetail", {
	...
	createContent : function(oController) {
		...
		var oToolbarLayout = new sap.ui.commons.layout.VerticalLayout({
			...
			content : [new sap.ui.core.Icon({src : "sap-icon://filter", size : "1.0rem", color : "#666666"}),
			           new sap.m.Label({text : oBundleText.getText("LABEL_2774") , design : "Bold"}).addStyleClass("L2PFontFamily"),	// 2774:검색조건
		...
		for(var i=0; i< vColumns.length; i++) {
			if(vColumns[i].control == "date"){
				oTable.addColumn(new sap.ui.table.Column({
					 label: new sap.ui.commons.Label({text: vColumns[i].label, textAlign : "Center"}).addStyleClass("L2PFontFamily"), 
		...
		var oResultPanel = new sap.m.Panel({
			...
			headerToolbar : new sap.m.Toolbar({
				...
				content : [new sap.ui.core.Icon({src : "sap-icon://table-chart", size : "1.2rem", color : "#666666"}),
				           new sap.m.Label({text : oBundleText.getText("LABEL_1467"), design : "Bold"}).addStyleClass("L2PFontFamily"),	// 1467:검색결과
				           new sap.m.ToolbarSpacer()]
			}).addStyleClass("L2PToolbarNoBottomLine"),
			content : [oPersonList]
		});
