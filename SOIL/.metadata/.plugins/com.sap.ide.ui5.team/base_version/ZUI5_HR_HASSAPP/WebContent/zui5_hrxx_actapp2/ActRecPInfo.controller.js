...
sap.ui.controller("zui5_hrxx_actapp2.ActRecPInfo", {
    ...
    vZipcodeColumns : [
                         {id : "Ichek", label : "선택", control : "Radio", width : 50, align : "Center", filter : "#rspan"},
                         {id : "Pstlz", label : "우편번호", control : "Text", width : 80, align : "Center", filter : "#rspan"},
		                 ...
		                 {id : "Ort02", label : "도로/건물번호", control : "Hidden", width : 10, align : "Left", filter : "#combo_filter"},
		                 {id : "State", label : "지역", control : "Hidden", width : 10, align : "Left", filter : "#text_filter"},
	...
	_Sub02TableJson : new sap.ui.model.json.JSONModel(), 
/**
...
//			this._vMolga = oEvent.data.Molga,
		...
		if(this._vActionType == "V") oTitle.setText("입사자정보조회");
		else if(this._vActionType == "M")  oTitle.setText("입사자정보수정");
		else oTitle.setText("입사자정보등록");
	    ...
	    if(oController._vHiringPersonalInfomationLayout == null || oController._vHiringPersonalInfomationLayout.length < 1) {
	    	sap.m.MessageBox.alert("Nothing Personal Information Layout this Molga (" + oController._vMolga + ")", {
	    		onClose : function() {
	    			oController.navToBack();
				}
	    	});	    	
			...
			for(var i=0; i<oCurIconTabbarItems.length; i++) {
				if(oCurIconTabbarItems[i].getCount() != "") {
					if(parseInt(oCurIconTabbarItems[i].getCount()) == 0) {
						...
						$("#" + oCurIconTabbarItems[i].getId() + "-count").css("color", "black");
...
//  저장버튼 클릭시....	
	...
	onPressAdd : function(oEvent) {
		...
		if(vMolga == "08" && oController._JobPage == "Sub24") {
			if(oController._vCntSub24 == 2) {
				sap.m.MessageBox.alert("두 개 이상의 은행 계정을 등록할 수 없습니다.");
				return;
			}
		}
		...
		if(vMolga == "10" && oController._JobPage == "Sub24") {
			if(oController._vCntSub24 == 7) {
				sap.m.MessageBox.alert("여덞 개 이상의 은행 계정을 등록할 수 없습니다.");
				return;
			}
		}
	...
	onPressModify : function(oEvent) {
		...
		if(oController._Sub02TableJson.getProperty("/Data") && oController._Sub02TableJson.getProperty("/Data").length > 0){
			if(vIDXs.length > 1){
				sap.m.MessageBox.alert("하나의 데이터만 수정이 가능합니다.");
				return;
			}else if(vIDXs.length < 1){
				sap.m.MessageBox.alert("수정할 데이터를 선택하여 주세요.");
				return;
			}
...
//		}
		...
		if(oController._Sub02TableJson.getProperty("/Data") && oController._Sub02TableJson.getProperty("/Data").length > 0){
//			if(vIDXs.length > 1){
//				sap.m.MessageBox.alert("하나의 데이터만 삭제가 가능합니다.");
//				return;
//			}else 
			if(vIDXs.length < 1){
				sap.m.MessageBox.alert("삭제할 데이터를 선택하여 주세요.");
				return;
			}

//			var _selPath = oTable.getContextByIndex(vIDXs[0]).sPath;
//			vContexts = oController._Sub02TableJson.getProperty(_selPath);
...
//		var oBkont = sap.ui.getCore().byId(oController.PAGEID + "_Sub24_Bkont");
		...
		oBkont.addItem(new sap.ui.core.Item({key : "0000", text : "-- 선택 --"}));
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	onDisplaySearchDialog : function(oEvent) {
		...
		if(oSlart.getSelectedKey() == "0000") {
			...
			sap.m.MessageBox.alert("교육기관을 선택해 주십시오.");
			return;
		}
...
//		var sValue = oEvent.getParameter("value");
	    ...
	    var oSlart = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Slart");
	    var sValue = sap.ui.getCore().byId(oController.PAGEID + "_POP_Schcd").getValue();
		if(sValue.length < 2){
			common.Common.showErrorMessage("검색조건은 2자 이상이어야 합니다.");
			return ;
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	onDisplaySearchZipcodeDialog : function(oEvent) {
		window.open("/sap/bc/ui5_ui5/sap/ZUI5_HR_HCOMMON/zip_search.html?CBF=fn_SetAddr", "pop", "width=550,height=550, scrollbars=yes, resizable=yes");
	},
		...
		for(var g=0; g<vGroupInfo.length; g++) {
			var oToolbar = new sap.m.Toolbar({
				...
				content : [ new sap.ui.core.Icon({
					        ...
					        new sap.m.Label({text : vGroupInfo[g].Itgrptx }).addStyleClass("L2PFontFamilyBold")]
...
//  학력사항을 가지고 온다.
	reloadSub02 : function() {
		...
		var oSub02Table = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_TABLE");
		var oDatas = { Data : []};
		var oPath = "/RecruitingSubjectsEducationSet/?$filter=Docno eq '" + oController._vDocno + "' and VoltId eq '" + oController._vVoltId  + "'";
		var oModel = sap.ui.getCore().getModel("ZL2P01GW0001_SRV");
		oModel.read(oPath,
				null,
				null,
...
//  학력사항 입력화면 Setting.
	setSub02 : function(oContext) {
		...
		var oSland = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Sland");
		var oSchcd = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Schcd");
		var oZzcolnm = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzcolnm");
		var oZzrecab = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzrecab");
		var oZzfinyn = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzfinyn");
		var oZzmajg1 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajg1");
		var oZzmajg2 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajg2"); 
		var oZzmajg3 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajg3");
		var oZzmajo1 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajo1");
		var oZzmajo2 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajo2"); 
		var oZzmajo3 = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzmajo3");
		var oZzrecab = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzrecab");
		var oZzfinyn = sap.ui.getCore().byId(oController.PAGEID + "_Sub02_Zzfinyn");
		// 값 초기화
		oBegda.setValue("");
    	...
    	oZzcolnm.setValue("");
    	oZzrecab.setSelected(false);
    	oZzfinyn.setSelected(false);
    	oZzmajg1.setSelectedItem(oZzmajg1.getFirstItem());
    	oZzmajg2.setSelectedItem(oZzmajg2.getFirstItem());
    	oZzmajg3.setSelectedItem(oZzmajg3.getFirstItem());
    	oZzmajo1.setValue("");
    	oZzmajo1.removeAllCustomData();
    	oZzmajo1.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo1", value : ""}));
    	oZzmajo2.setValue("");
    	oZzmajo2.removeAllCustomData();
    	oZzmajo2.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo2", value : ""}));
    	oZzmajo3.setValue("");
    	oZzmajo3.removeAllCustomData();
    	oZzmajo3.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo3", value : ""}));
    	oZzrecab.setSelected(false );
    	oZzfinyn.setSelected(false );
    	oController.onClearFaartFields(oController);
		...
		oZzrecab.setEnabled(!oController._DISABLED);
		oZzfinyn.setEnabled(!oController._DISABLED);
		oZzmajg1.setEnabled(!oController._DISABLED);
		oZzmajg2.setEnabled(!oController._DISABLED);
		oZzmajg3.setEnabled(!oController._DISABLED);
		oZzmajo1.setEnabled(!oController._DISABLED);
		oZzmajo2.setEnabled(!oController._DISABLED);
		oZzmajo3.setEnabled(!oController._DISABLED);
		
		if(oContext != null) {
			//Global 일자 관련하여 소스 수정함. 2015.10.19
			oBegda.setValue(dateFormat.format(new Date(common.Common.setTime(new Date(oContext.Begda)))));
			//수정완료
			oBegda.removeAllCustomData();
			oBegda.addCustomData(new sap.ui.core.CustomData({key : "Seqnr", value : oContext.Seqnr}));
			
			//Global 일자 관련하여 소스 수정함. 2015.10.19
			oEndda.setValue(dateFormat.format(new Date(common.Common.setTime(new Date(oContext.Endda)))));
			//수정완료
			...
			oSchcd.setValue(oContext.Schtx);
			oSchcd.removeAllCustomData();
	    	oSchcd.addCustomData(new sap.ui.core.CustomData({key : "Schcd", value : oContext.Zzschcd}));
	    	if(oContext.Zzmajg1){
	    		oZzmajg1.setSelectedKey(oContext.Zzmajg1);
	    	}
	    	if(oContext.Zzmajg2){
	    		oZzmajg2.setSelectedKey(oContext.Zzmajg2);
	    	}
	    	if(oContext.Zzmajg3){
	    		oZzmajg3.setSelectedKey(oContext.Zzmajg3);
	    	}
	    	oZzmajo1.setValue(oContext.Zzmajo1tx);
	    	oZzmajo1.removeAllCustomData();
	    	oZzmajo1.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo1", value : oContext.Zzmajo1}));
	    	oZzmajo2.setValue(oContext.Zzmajo2tx);
	    	oZzmajo2.removeAllCustomData();
	    	oZzmajo2.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo2", value : oContext.Zzmajo2}));
	    	oZzmajo3.setValue(oContext.Zzmajo3tx);
	    	oZzmajo3.removeAllCustomData();
	    	oZzmajo3.addCustomData(new sap.ui.core.CustomData({key : "Zzmajo3", value : oContext.Zzmajo3}));
	    	oZzcolnm.setValue(oContext.Zzcolnm);
	    	oZzrecab.setSelected(oContext.Zzrecab == "X" ? true : false );
	    	oZzfinyn.setSelected(oContext.Zzfinyn == "X" ? true : false );
		}
	},
...
//		oSltp1.setValueState(sap.ui.core.ValueState.None);
		...
		var vOneData = {
				...
				Zzschcd : oSchcd.getCustomData().length ? oSchcd.getCustomData()[0].getValue("Schcd") : "",
				Zzmajg1 : oZzmajg1.getSelectedKey(),
				Zzmajg2 : oZzmajg2.getSelectedKey(),
				Zzmajg3 : oZzmajg3.getSelectedKey(),
				Zzmajo1 : oZzmajo1.getCustomData()[0].getValue(),
				Zzmajo2 : oZzmajo2.getCustomData()[0].getValue(),
				Zzmajo3 : oZzmajo3.getCustomData()[0].getValue(),
				Zzrecab : oZzrecab.getSelected() == true ? "X" : "" ,
				Zzfinyn : oZzfinyn.getSelected() == true ? "X" : "" ,
				Zzcolnm : oZzcolnm.getValue()		
		};
		
//		// 입학일자(Begda)
//		if(vOneData.Begda == null) {
//			oBegda.setValueState(sap.ui.core.ValueState.Error);
//			sap.m.MessageBox.alert("입학일을 입력바랍니다.");
//			return;
//		}
//		
//		// 졸업일자(Endda)
...
//			sap.m.MessageBox.alert("졸업일을 입력바랍니다.");
//			return;
//		}
//		
...
//			oBegda.setValueState(sap.ui.core.ValueState.Error);
//			oEndda.setValueState(sap.ui.core.ValueState.Error);
//			sap.m.MessageBox.alert("입학일이 졸업일보다 큽니다");
//			return;
//		}
//		
//		// 교육기관(Slart)
...
//			sap.m.MessageBox.alert("학위를 선택해 주십시오.");
//			return;
//		}
//		
//		// 학교(Schcd)
...
//			sap.m.MessageBox.alert("학교명를 선택해 주십시오");
//			return;
//		}
//
//		// 국가(Sland)
...
//			sap.m.MessageBox.alert("국가를 선택해 주십시오.");
//			return;
//		}
//		
...
//				Zzlndep	: oZzlndep.getValue(),	
		...
		if(new Date(oBegda.getValue()) > new Date(oController._vActda)) {
			oBegda.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("Join Date must be earlier than the Action Date.");
			return;
		}
		
		if(new Date(oEndda.getValue()) > new Date(oController._vActda)) {
			oEndda.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("Leave Date must be earlier than the Action Date.");
			return;
		}
		...
		if(new Date(oBegda.getValue()) > new Date(oEndda.getValue())) {
			oBegda.setValueState(sap.ui.core.ValueState.Error);
			oEndda.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("입사일자가 퇴사일자보다 큽니다");
			return;
		}
		...
		if(vOneData.Arbgb == "") {
			...
			sap.m.MessageBox.alert("직접 입력은 원하는 회사가 목록에 존재하지 않는 경우에만 사용하십시오.");
			return;
		}
...
//	어학사항 저장
	saveSub04 : function() {
		...
		if(vOneData.Quali == "") {
			...
			sap.m.MessageBox.alert("언어구분을 선택하세요");
			return;
		}
		...
		if(vOneData.Exmty == "") {
			...
			sap.m.MessageBox.alert("시험구분을 선택해 주십시오.");
			return;
		}
		...
		if(vOneData.Eamdt == null) {
			...
			sap.m.MessageBox.alert("시험일을 입력바랍니다.");
			return;
		}
		...
		if(vOneData.Exmto == null) {
			...
			sap.m.MessageBox.alert("만료일을 입력바랍니다.");
			return;
		}
		...
		if(new Date(oEamdt.getValue()) > new Date(oExmto.getValue())) {
			oEamdt.setValueState(sap.ui.core.ValueState.Error);
			oExmto.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("The examination date is prior to the expiration date.");
			return;
		}
		...
		if(vOneData.Exmsc == null && vOneData.Eamgr == "") {
			...
			sap.m.MessageBox.alert("점수 또는 등급을 입력바랍니다.");
			return;
		}
...
//	자격면허 저장
	saveSub06 : function() {
		...
		if(vOneData.Cttyp == "" && vOneData.Cttyptx == "") {
			...
			sap.m.MessageBox.alert("자격증 정보를 입력해 주십시오.");
			return;
		}
...
//			sap.m.MessageBox.alert("자격증번호를 입력바랍니다.");
//			return;
//		}
		...
		if(vOneData.Ctend != null && new Date(oCerda.getValue()) > new Date(oCtend.getValue())){
			...
			sap.m.MessageBox.alert("취득일이 만료일보다 큽니다");
			return;
		}
...
//  병역사항을 저장 한다.	
	saveSub07 : function() {
		...
		if(vOneData.Begda == null) {
			oBegda.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("입대일자를 입력바랍니다.");
			return;
		}
		...
		if(vOneData.Endda == null) {
			oEndda.setValueState(sap.ui.core.ValueState.Error);
			sap.m.MessageBox.alert("전역일자를 입력바랍니다.");
			return;
		}
		...
		if(vOneData.Serty == "") {
			...
			sap.m.MessageBox.alert("병역유형을 선택해 주십시오.");
			return;
		}
		...
		if(vOneData.Preas == "") {
			...
			sap.m.MessageBox.alert("제대구분을 선택해 주십시오");
			return;
		}
		...
		if(vOneData.Jobcl == "") {
			...
			sap.m.MessageBox.alert("보직분류를 선택해 주십시오.");
			return;
		}
		...
		if(vOneData.Mrank == "") {
			...
			sap.m.MessageBox.alert("계급을 선택해 주십시오.");
			return;
		}
	...
	deleteSub07 : function(oEvent) {
		...
		if(oController._vCntSub07 == 0) {
			sap.m.MessageBox.alert("삭제할 데이타가 없습니다.");
			return;
		};
...
//  주소정보사항을 가지고 온다.
	readSub21 : function() {
		...
		if(oController._vAnssaList.length < 1) {
			sap.m.MessageBox.alert("등록 가능한 주소유형이 없습니다.");
			return;
		}
		
		if(oController._vAddressLayout.length < 1) {
			sap.m.MessageBox.alert("주소유형별 컬럼정보가 없습니다.");
			return;
		}
	...
	onDeleteEmecAddress : function(oEvent) {
		...
		if(oController._vEmecAddressListCount < 2) {
			sap.m.MessageBox.alert("긴급주소는 최소한 1개 이상 존재해야 하기때문에 삭제할 수 없습니다.");
			return;
		}
					...
					if(vSubty == oController._vAddressLayout[i].Subty) {
						...
						if(Fieldtype == "M1") {
							...
							if(vVal == "" || vVal == "0000") {
								...
								var vMsg = "&Cntl 를(을) 선택해 주시기 바랍니다.";
								vMsg = vMsg.replace("&Cntl", oController._vAddressLayout[i].Label);
								sap.m.MessageBox.alert(vMsg);
								return null;
							}
						} else if(Fieldtype == "M3") {
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	onSetFaartCodeFragment : function(oEvent) {
		...
		if(oController._OFaartCodeControl.sId.indexOf("Zzmajo1") > 0){
			...
			oDialog.setTitle("복수전공");
		}else{
			oDialog.setTitle("부전공");
		}			
		...
		if(!oFaartCodeModel.getProperty("/FaartCodeListSet") || oFaartCodeModel.getProperty("/FaartCodeListSet").length < 1){
			...
			var oPath = "/EmpCodeListSet/?$filter=Field eq 'Zzmajo1' and Persa eq '" + oController._vPersa + "'";
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	onConfirmRehire : function(oEvnet){
		...
		if(!oProcessResult){
			sap.m.MessageBox.alert("해당하는 신분증 번호의 대상을 찾지 못하였습니다.");
			return ;
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////	
	...
	onDisplaySearchInputDialog : function(oEvent) {
		...
		if(oCustomData[0]){
			...
			if(vSearchInputType == "Schcd"){
				oInputDialog.setTitle("학교");
				oInputNotice.setText("직접 입력은 원하는 학교가 목록에 존재하지 않는 경우에만 사용하십시오.");
				...
				oInputDialog.setTitle("전공");
				oInputNotice.setText("직접 입력은 원하는 전공1이 목록에 존재하지 않는 경우에만 사용하십시오.");
				...
				oInputDialog.setTitle("부전공");
				oInputNotice.setText("직접 입력은 원하는 전공2가 목록에 존재하지 않는 경우에만 사용하십시오.");
				...
				oInputDialog.setTitle("자격증 유형"); //
				oInputNotice.setText("직접 입력은 원하는 자격증 유형이 목록에 존재하지 않는 경우에만 사용하십시오.");
				...
				oInputDialog.setTitle("이전회사명"); //
				oInputNotice.setText("직접 입력은 원하는 회사가 목록에 존재하지 않는 경우에만 사용하십시오.");
	...
	displayCodeSearchDialog : function(oEvent) {
		...
		var vEmpCodeList = mEmpCodeList.getProperty("/EmpCodeListSet");
		if(vEmpCodeList && vEmpCodeList.length) {
			vOneCodeList.EmpCodeListSet.push({Field : Fieldname, Ecode : "", Etext : "값 없음"});
			for(var i=0; i<vEmpCodeList.length; i++) {
...
//	},
	...
	onChangeSSN : function(oEvent) {
		...
		var t2 = s;
		if(isNaN(t1)) {
			sap.m.MessageBox.alert("숫자만 입력가능합니다.");
			return;
		}
...
getDomainValueData : function(Fieldname, vAddFilter, select) {
	...
	SearchQualiDialog : function(oEvent){
		...
		vCode.ZzqualiCodeListSet.push({Ecode : "", Etext : "값 없음"});
	...
	onConfirmQuali : function(oEvent){
		...
		if (aContexts.length) {
	    	...
	    	if(vQualitx == "값 없음") oZzquali.setValue("");
...
function fn_SetAddr(Zip, Addr) {
	...
	var Bezei = "";
	var Ort01 = "";
	var oLand1 = sap.ui.getCore().byId(oController.PAGEID + "_Sub01_Land1");
	var oPstlz = sap.ui.getCore().byId(oController.PAGEID + "_Sub01_Pstlz");
	var oState = sap.ui.getCore().byId(oController.PAGEID + "_Sub01_State");
	var oOrt01 = sap.ui.getCore().byId(oController.PAGEID + "_Sub01_Zzaddr1_kr");
		...
		oModel.create(
			    ...
			    function (oData, response) {
					if(oData) {
						...
						oPstlz.setValue(Pstlz);
						if(oState) {
							oState.setSelectedKey(oData.State)
						}
						if(oOrt01) oOrt01.setValue(oData.Zzaddr1Kr);
					}
...
//	if(index > -1) {
//		Bezei = Addr1.substring(0,index);
//		Ort01 = Addr1.substring(index+1);
//	} else {
//		Bezei = "";
//		Ort01 = Addr1;
//	}
//	
//	sap.ui.getCore().byId(oController.PAGEID +_" + Gubun + "_Pstlz").setValue(Pstlz);
//	sap.ui.getCore().byId(oController.PAGEID + "_Bezei").setValue(Bezei);
//	sap.ui.getCore().byId("ZKL_PA_EmpProfile_1020_KR_" + Gubun + "_Ort01").setValue(Ort01);
//	sap.ui.getCore().byId("ZKL_PA_EmpProfile_1020_KR_" + Gubun + "_Ort02").setValue(Ort02);
