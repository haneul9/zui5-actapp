	...
	onESSelectPerson : function(oEvent) {
		...
		if(vEmpSearchResult && vEmpSearchResult.length) {
			...
			for(var i=0; i<vIDXs.length; i++) {
				var vPersa = mEmpSearchResult.getProperty("/EmpSearchResultSet/" + vIDXs[i] + "/Persa");
				if(oController._vPersa != vPersa) {
					sap.m.MessageBox.alert("선택된 사용자는 동일한 인사영역이 아닙니다.");
					return;
				}
				
				var vPersg = mEmpSearchResult.getProperty("/EmpSearchResultSet/" + vIDXs[i] + "/Persg");
				if(vPersg != "9") vPersg = "1";
				if(vPrev_Persg != "" && vPrev_Persg != vPersg) {
					sap.m.MessageBox.alert("선택된 사용자는 동일한 사원그룹이 아닙니다.");
					return;
				}
				vPrev_Persg = vPersg;
				...
				vActionSubjectList_Temp.ActionSubjectListSet.push({
					...
					Persg : mEmpSearchResult.getProperty("/EmpSearchResultSet/" + vIDXs[i] + "/Persg"),
				}); 
			}
...
//			}
	...
	onPressSave : function(oEvent) {
		...
		if(vCreateData.Massn1 == "" || vCreateData.Massg1 == "") {
			sap.m.MessageBox.alert("발령유형 또는 발령사유를 선택해 주시기 바랍니다.");
			return;
		}
