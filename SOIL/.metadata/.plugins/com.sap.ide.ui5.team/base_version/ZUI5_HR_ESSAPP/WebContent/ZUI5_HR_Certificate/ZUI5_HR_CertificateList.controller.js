...
sap.ui.controller("ZUI5_HR_Certificate.ZUI5_HR_CertificateList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_Certificate.ZUI5_HR_CertificateList
	 */
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1){
			if(_gAuth == "E"){
				...
				var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , Pernr : "", Ename : "" , Payym :"", Encid : ""}};
				oController._ListCondJSonModel.setData(JSonData);
			}
		}
	...
	onPressNew : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Certificate.ZUI5_HR_CertificateList");
		var oController = oView.getController();
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_Certificate.ZUI5_HR_CertificateDetail",
		      data : {
		    	  Appno : "" ,
		      }
		});	
	...
	onPressSearch : function(oEvent){
		...
		if(SearchCond.Encid != ""){
			filterString = filterString + "%20and%20" + "Encid eq '" + encodeURIComponent(SearchCond.Encid) + "'";
		}
...
//			}
	...
	onPrintCert : function(oEvent){
		...
		var vPrcty = oEvent.getSource().getCustomData()[2].getValue();
		var vErrorMessage = "";
		if(vPrcty == "Z" && vCerty && (vCerty == "B3" || vCerty == "B4")){ 
		...
		var vInfoTxt = "" , vCompTxt = "";
			...
			if(vPrcty == "V"){
				vInfoTxt = oBundleText.getText("LABEL_2899");	// 2899 :미리보기를 하시겠습니까?
			}
