...
sap.ui.controller("ZUI5_HR_DeductionItem.ZUI5_HR_DeductionItemList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_DeductionItem.ZUI5_HR_DeductionItemList
	 */

	PAGEID : "ZUI5_HR_DeductionItemList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			...
			var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , Encid : vEncid, Ename : vEname, Dedgb : "ALL" }};
			oController._ListCondJSonModel.setData(JSonData);
			
			// 메뉴얼 버튼 활성화
			common.Common.setInformationButton(oController, "A");
		} else {
			var vDedgb = oController._ListCondJSonModel.getProperty("/Data/Dedgb") || 'ALL';
			oController._ListCondJSonModel.setProperty("/Data/Dedgb", vDedgb);
		}
		...
		// 변동/고정공제 combo
		oController.onSetDedgb(oController);
	},
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_DeductionItem.ZUI5_HR_DeductionItemList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : "ZUI5_HR_DeductionItem.ZUI5_HR_DeductionItemDetail",
			data : { Appno : _Appno }
		});
	},
	
	onSetDedgb : function(oController) {
		var oModel = sap.ui.getCore().getModel("ZHR_DEDUCT_OBJ_SRV"),
			oDedgb = sap.ui.getCore().byId(oController.PAGEID + "_Dedgb");
	
		if(oDedgb.getItems()) oDedgb.destroyItems();
		
		oDedgb.addItem(new sap.ui.core.Item({ key : "ALL", text : oBundleText.getText("LABEL_0212") }));	// 212:전체
		
		oModel.read("/DedGbnListSet", {
			async : false,
			success : function(data, res) {
				if(data && data.results.length) {
					data.results.forEach(function(elem) {
						oDedgb.addItem(new sap.ui.core.Item({
							key : elem.Dedgb,
							text : elem.Dedgbtx
						}));
					});
	...
	onPressSearch : function(oEvent) {
		...
		if(SerchCond.Dedgb != "" && SerchCond.Dedgb != "ALL") {
			aFilters.push(new sap.ui.model.Filter('Dedgb', sap.ui.model.FilterOperator.EQ, SerchCond.Dedgb));
		}
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.Reqym = (!OneData.Reqym || OneData.Reqym == "000000") ? "" : OneData.Reqym.replace(/(\d{4})(\d{2})/g, '$1.$2');
							OneData.Pdeym = (!OneData.Pdeym || OneData.Pdeym == "000000") ? "" : OneData.Pdeym.replace(/(\d{4})(\d{2})/g, '$1.$2');
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0457") + "-" + dateFormat.format(curDate) + ".xlsx"	// 457:고정/변동 공제신청 (항목)
		};
