jQuery.sap.declare("ZUI5_HR_CommuteRecord.control.OrgSearch");
...
ZUI5_HR_CommuteRecord.control.OrgSearch = {
	/** 
	* @memberOf ZUI5_HR_CommuteRecord.control.OrgSearch
	*/	
	...
	OrgSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecord.ZUI5_HR_CommuteRecordList");
...
//			oTable.bindItems("/OrgListSet");
	...
	onClose : function(oEvent) {
		...
		if(oControl){
			oControl.setValue("");
			oController._ListCondJSonModel.setProperty("/Data/Corgeh", undefined); 
			oController._ListCondJSonModel.setProperty("/Data/Corgtx", undefined); 
		}
	...
	onKeyUp : function(oEvent) {
		if(oEvent.which == 13) {
			ZUI5_HR_CommuteRecord.control.OrgSearch.OrgSearch();
		}
	},
	...
	onConfirm : function(oEvent) {
		...
		if(oInput) {
			if(vSelectedOrg && vSelectedOrg.length) {
				...
				oController._ListCondJSonModel.setProperty("/Data/Corgeh", vSelectedOrg[0].Orgeh); 
				oController._ListCondJSonModel.setProperty("/Data/Corgtx", vSelectedOrg[0].Stext); 
			}
		}
	...
	onClick : function(oEvent) {
		...
		ZUI5_HR_CommuteRecord.control.OrgSearch.clicks = ZUI5_HR_CommuteRecord.control.OrgSearch.clicks + 1;
		
		if(ZUI5_HR_CommuteRecord.control.OrgSearch.clicks == 1) {
			setTimeout( ZUI5_HR_CommuteRecord.control.OrgSearch.clearClicks, 500);
		} else if(ZUI5_HR_CommuteRecord.control.OrgSearch.clicks == 2) {
			ZUI5_HR_CommuteRecord.control.OrgSearch._vSPath = vContext.sPath;
		}
	},
	...
	clearClicks : function () {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecord.ZUI5_HR_CommuteRecordList");
		var oController = oView.getController();
		ZUI5_HR_CommuteRecord.control.OrgSearch.clicks = 0;
		ZUI5_HR_CommuteRecord.control.OrgSearch._vSPath = -1;
	},
	...
	onDblClick : function(oEvent){
		...
		var oController = oView.getController();
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_OrgSearch_TABLE");
		if(ZUI5_HR_CommuteRecord.control.OrgSearch._vSPath == null || ZUI5_HR_CommuteRecord.control.OrgSearch._vSPath == -1) return ;
		var vIndex = ZUI5_HR_CommuteRecord.control.OrgSearch._vSPath.replace("/OrgListSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		ZUI5_HR_CommuteRecord.control.OrgSearch.onConfirm();
	},
