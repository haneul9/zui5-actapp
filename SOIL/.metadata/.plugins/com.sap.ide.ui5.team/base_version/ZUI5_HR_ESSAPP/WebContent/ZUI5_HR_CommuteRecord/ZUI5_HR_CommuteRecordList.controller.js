...
sap.ui.controller("ZUI5_HR_CommuteRecord.ZUI5_HR_CommuteRecordList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_CommuteRecord.ZUI5_HR_CommuteRecordList
	 */

	PAGEID : "ZUI5_HR_CommuteRecordList",
...
//		this.onPressSearch();
	...
	onPressSearch : function(oEvent) {
		...
		function Search() {
			...
			oModel.read("/InOutRecordDataSet", {
				async: false,
				filters: aFilters,
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = vIdx++;
							OneData.Rbeguz = common.Common.timeToString(OneData.Rbeguz, "HH:mm");
							OneData.Inptim = common.Common.timeToString(OneData.Inptim, "HH:mm");
							OneData.Diftim = common.Common.timeToString(OneData.Diftim, "HH:mm");
							OneData.Outtim = common.Common.timeToString(OneData.Outtim, "HH:mm");
	...
	displayOrgehSearchDialog : function(oEvent) {
		...
		if(!oController._SerachOrgDialogInView) {
			oController._SerachOrgDialogInView = sap.ui.jsfragment("ZUI5_HR_CommuteRecord.fragment.OrgSearch", oController);
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0584") + "-" + dateFormat.format(curDate) + ".xlsx"	// 584:출퇴근 기록 조회
		};
