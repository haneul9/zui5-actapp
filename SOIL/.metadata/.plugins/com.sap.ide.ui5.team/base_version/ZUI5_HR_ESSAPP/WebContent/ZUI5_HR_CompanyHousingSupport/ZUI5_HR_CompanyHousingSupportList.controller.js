...
sap.ui.controller("ZUI5_HR_CompanyHousingSupport.ZUI5_HR_CompanyHousingSupportList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_CompanyHousingSupport.ZUI5_HR_CompanyHousingSupportList
	 */

	PAGEID : "ZUI5_HR_CompanyHousingSupportList",
		...
		if(!oController._ListCondJSonModel.getProperty("/Data")){
			if(vEmpLoginInfo.length > 0){
				...
				if(_gAuth == "E"){
					...
					vEncid = vEmpLoginInfo[0].Pernr ;
				}
			}
			
			var JSonData = {Data : {Apbeg : dateFormat.format(prevDate), Apend : dateFormat.format(curDate), Encid : vEncid, Ename : vEname, Auth : _gAuth}};
...
//		if(vData != undefined) oController.onGetCountDown(oController ,vData.Pernr );
	...
	onPressRegCheck : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_CompanyHousingSupport.ZUI5_HR_CompanyHousingSupportList"),
	...
	onPressNew : function(oEvent){
		...
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_CompanyHousingSupport.ZUI5_HR_CompanyHousingSupportDetail",
		      data : {Appno : ""}
		});	
	},
	...
	onPressSearch : function(oEvent){
		...
		aFilters.push(new sap.ui.model.Filter('Begbn', sap.ui.model.FilterOperator.EQ, "B"));
		...
		if(SerchCond.Conbg != undefined && SerchCond.Conbg != "") {
			aFilters.push(new sap.ui.model.Filter('Conbg', sap.ui.model.FilterOperator.EQ, common.Common.adjustGMT(sap.ui.getCore().byId(oController.PAGEID + "_Conbg").getDateValue())));
		}
		if(SerchCond.Coned != undefined && SerchCond.Coned != "") {
			aFilters.push(new sap.ui.model.Filter('Coned', sap.ui.model.FilterOperator.EQ, common.Common.adjustGMT(sap.ui.getCore().byId(oController.PAGEID + "_Coned").getDateValue())));
		}
		if(SerchCond.Encid != "") {
...
//			}
	...
	onESSelectPerson : function(oEvent) {
		...
		if(vEmpSearchResult && vEmpSearchResult.length > 0 ) {
			...
			oController._ListCondJSonModel.setProperty("/Data/Pernr", mEmpSearchResult.getProperty(_selPath + "/Encid"));
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0320") + "-" + dateFormat.format(curDate) + ".xlsx"	// 320:사택지원 신청
			};
