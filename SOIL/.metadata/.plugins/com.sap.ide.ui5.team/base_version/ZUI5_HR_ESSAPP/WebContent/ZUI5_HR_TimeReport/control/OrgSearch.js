jQuery.sap.declare("ZUI5_HR_TimeReport.control.OrgSearch");
...
ZUI5_HR_TimeReport.control.OrgSearch = {
	/** 
	* @memberOf ZUI5_HR_TimeReport.control.OrgSearch
	*/	
	...
	OrgSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_TimeReport.ZUI5_HR_TimeReportList");
...
//			oTable.bindItems("/OrgListSet");
	...
	onClose : function(oEvent) {
		...
		if(oControl){
			...
			oController._ListCondJSonModel.setProperty("/Data/Orgehtx", undefined); 
		}
	...
	onKeyUp : function(oEvent) {
		if(oEvent.which == 13) {
			ZUI5_HR_TimeReport.control.OrgSearch.OrgSearch();
		}
	},
	...
	onConfirm : function(oEvent) {
		...
		if(oInput) {
			if(vSelectedOrg && vSelectedOrg.length) {
				...
				oController._ListCondJSonModel.setProperty("/Data/Orgehtx", vSelectedOrg[0].Stext); 
			}
		}
	...
	onClick : function(oEvent) {
		...
		ZUI5_HR_TimeReport.control.OrgSearch.clicks = ZUI5_HR_TimeReport.control.OrgSearch.clicks + 1;
		
		if(ZUI5_HR_TimeReport.control.OrgSearch.clicks == 1) {
			setTimeout( ZUI5_HR_TimeReport.control.OrgSearch.clearClicks, 500);
		} else if(ZUI5_HR_TimeReport.control.OrgSearch.clicks == 2) {
			ZUI5_HR_TimeReport.control.OrgSearch._vSPath = vContext.sPath;
		}
	},
	...
	clearClicks : function () {
		var oView = sap.ui.getCore().byId("ZUI5_HR_TimeReport.ZUI5_HR_TimeReportList");
		var oController = oView.getController();
		ZUI5_HR_TimeReport.control.OrgSearch.clicks = 0;
		ZUI5_HR_TimeReport.control.OrgSearch._vSPath = -1;
	},
	...
	onDblClick : function(oEvent){
		...
		var oController = oView.getController();
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_OrgSearch_TABLE");
		if(ZUI5_HR_TimeReport.control.OrgSearch._vSPath == null || ZUI5_HR_TimeReport.control.OrgSearch._vSPath == -1) return ;
		var vIndex = ZUI5_HR_TimeReport.control.OrgSearch._vSPath.replace("/OrgListSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		ZUI5_HR_TimeReport.control.OrgSearch.onConfirm();
	},
