...
sap.ui.controller("ZUI5_HR_EntryRecord.ZUI5_HR_EntryRecordList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_EntryRecord.ZUI5_HR_EntryRecordList
	 */

	PAGEID : "ZUI5_HR_EntryRecordList",
...
//		this.onPressSearch();
		...
		sap.ui.getCore().byId(this.PAGEID + "_Table").setVisibleRowCount(0);
	},
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = vIdx++;
							OneData.Iotim = common.Common.timeToString(OneData.Iotim, "HH:mm:ss");
	...
	displayOrgehSearchDialog : function(oEvent) {
		...
		if(!oController._SerachOrgDialogInView) {
			oController._SerachOrgDialogInView = sap.ui.jsfragment("ZUI5_HR_EntryRecord.fragment.OrgSearch", oController);
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0575") + "-" + dateFormat.format(curDate) + ".xlsx"	// 575:출퇴근 기록 조회
		};
