...
sap.ui.controller("ZUI5_HR_MoveAmount.ZUI5_HR_MoveAmountList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_MoveAmount.ZUI5_HR_MoveAmountList
	 */

	PAGEID : "ZUI5_HR_MoveAmountList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			...
			var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , Pernr : vPernr, Encid : vEncid, Ename : vEname }};
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_MoveAmount.ZUI5_HR_MoveAmountList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_MoveAmount.ZUI5_HR_MoveAmountDetail",
		      data : { Appno : _Appno }
		});
	},
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0; i<data.results.length; i++) {
							...
							OneData.Aplamt = common.Common.numberWithCommas(OneData.Aplamt);
...
//			}
	...
	onExport : function(oEvent) {
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0278") + "-" + dateFormat.format(curDate) + ".xlsx"	// 278:이사비 신청
			};
