sap.ui.jsfragment("ZUI5_HR_Transition.fragment.SearchDialog", {

	createContent : function(oController) {	
		jQuery.sap.require("ZUI5_HR_Transition.control.ObjectSearch");
		...
		var oFilterBar = new sap.m.Toolbar({
			content : [ 
				...
				new sap.m.Input(oController.PAGEID + "_SearchDialog_Text", {
					width: "150px",
					value : "{Keyword}",
					change : ZUI5_HR_Transition.control.ObjectSearch.onKeyUp
				}).addStyleClass("Font14px FontColor3"),
				new sap.m.ToolbarSpacer({width: "10px"}),
				new sap.m.Button({
					text : oBundleText.getText("LABEL_0002"),
					press : ZUI5_HR_Transition.control.ObjectSearch.onKeyUp
				}),
				new sap.m.ToolbarSpacer({width: "10px"})
			]
		...
		var oDialog = new sap.m.Dialog(oController.PAGEID + "_SearchDialog", {
			...
			showHeader : true,
			title : "{Title}",
			beforeOpen : ZUI5_HR_Transition.control.ObjectSearch.onBeforeOpenSearchDialog,
			afterOpen : ZUI5_HR_Transition.control.ObjectSearch.onAfterOpenSearchDialog,
			afterClose : ZUI5_HR_Transition.control.ObjectSearch.onAfterCloseSearchDialog,
			beginButton :  new sap.m.Button({
				icon : "sap-icon://accept",
				text : oBundleText.getText("LABEL_0037"),	// 37:선택
				press : ZUI5_HR_Transition.control.ObjectSearch.onConfirmSearchDialog}),
			endButton : new sap.m.Button({
				icon : "sap-icon://sys-cancel-2",
				text :  oBundleText.getText("LABEL_0071"),	// 71:취소
				press : ZUI5_HR_Transition.control.ObjectSearch.onCloseSearchDialog}),
			content : [oLayout]
		}).setModel(oJSonModel);
