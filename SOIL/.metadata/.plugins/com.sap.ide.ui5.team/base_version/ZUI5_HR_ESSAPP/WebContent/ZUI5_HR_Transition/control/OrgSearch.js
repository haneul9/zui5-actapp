jQuery.sap.declare("ZUI5_HR_Transition.control.OrgSearch");
...
ZUI5_HR_Transition.control.OrgSearch = {
	/** 
...
//			oTable.bindItems("/OrgListSet");
	...
	onClose : function(oEvent) {
		...
		if(oControl){
			oControl.setValue("");
			oController._DetailJSonModel.setProperty("/Data/Hoorgeh", undefined); 
			oController._DetailJSonModel.setProperty("/Data/Hoorgtx", undefined); 
		}
	...
	onKeyUp : function(oEvent) {
		if(oEvent.which == 13) {
			ZUI5_HR_Transition.control.OrgSearch.OrgSearch();
		}
	},
	...
	onConfirm : function(oEvent) {
		...
		if(oInput) {
			if(vSelectedOrg && vSelectedOrg.length) {
				...
				oController._DetailJSonModel.setProperty("/Data/Hoorgeh", vSelectedOrg[0].Orgeh); 
				oController._DetailJSonModel.setProperty("/Data/Hoorgtx", vSelectedOrg[0].Stext); 
			}
		}
	...
	onClick : function(oEvent) {
		...
		ZUI5_HR_Transition.control.OrgSearch.clicks = ZUI5_HR_Transition.control.OrgSearch.clicks + 1;
	        
	     if(ZUI5_HR_Transition.control.OrgSearch.clicks == 1) {
	          setTimeout( ZUI5_HR_Transition.control.OrgSearch.clearClicks, 500);
	     } else if(ZUI5_HR_Transition.control.OrgSearch.clicks == 2) {
	    	 ZUI5_HR_Transition.control.OrgSearch._vSPath = vContext.sPath;
	     }
	},
	...
	clearClicks : function () {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Transition.ZUI5_HR_TransitionDetail");
		var oController = oView.getController();
		ZUI5_HR_Transition.control.OrgSearch.clicks = 0;
		ZUI5_HR_Transition.control.OrgSearch._vSPath = -1;
	...
	onDblClick : function(oEvent){
		...
		var oController = oView.getController();
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_OrgSearch_TABLE");
		if(ZUI5_HR_Transition.control.OrgSearch._vSPath == null || ZUI5_HR_Transition.control.OrgSearch._vSPath == -1) return ;
		var vIndex = ZUI5_HR_Transition.control.OrgSearch._vSPath.replace("/OrgListSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		ZUI5_HR_Transition.control.OrgSearch.onConfirm();
	},
