...
sap.ui.controller("ZUI5_HR_Smock.ZUI5_HR_SmockList", {
	/**
	...
	onBeforeShow : function(oEvent) {
		var oController = this,
			...
			curDate = new Date(),
			prevDate = new Date(curDate.getFullYear() - 2, curDate.getMonth(), curDate.getDate()+1),
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			...
			var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , Encid : vEncid, Ename : vEname, Sgubn : "All" }};
			oController._ListCondJSonModel.setData(JSonData);
			
			// 메뉴얼 버튼 활성화
			common.Common.setInformationButton(oController, "A");
		} else {
			var vSgubn = oController._ListCondJSonModel.getProperty("/Data/Sgubn") || 'All';
			oController._ListCondJSonModel.setProperty("/Data/Sgubn", vSgubn);
		}
		
		// 신청구분 Combo
		oController.onSetSgubn(oController);
	},
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Smock.ZUI5_HR_SmockList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_Smock.ZUI5_HR_SmockDetail",
		      data : { Apdoc : _Apdoc }
		});
	},
	
	onSetSgubn : function(oController) {
		var oModel = sap.ui.getCore().getModel("ZHR_BSUIT_SRV"),
			oSgubn = sap.ui.getCore().byId(oController.PAGEID + "_Sgubn");
		
		if(oSgubn.getItems()) oSgubn.destroyItems();
		
		oSgubn.addItem(new sap.ui.core.Item({
			key : "All",
			text : oBundleText.getText("LABEL_0212")	// 212:전체
		}));
		
		oModel.read("/SgubnCodeSet", {
			async : false,
	...
	// 신규신청 로직
	onPressWinterNew : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Smock.ZUI5_HR_SmockList"),
			oController = oView.getController();
		
		// 신청전 체크
		if(oController.preRegCheck(oController, '10')) {
			sap.ui.getCore().getEventBus().publish("nav", "to", {
				id : "ZUI5_HR_Smock.ZUI5_HR_SmockDetail",
				data : {Apdoc : "", Sgubn : '10'}
			});
	...
	onPressSummerNew : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Smock.ZUI5_HR_SmockList"),
			oController = oView.getController();
		
		// 신청전 체크
		if(oController.preRegCheck(oController, '20')) {
			sap.ui.getCore().getEventBus().publish("nav", "to", {
				id : "ZUI5_HR_Smock.ZUI5_HR_SmockDetail",
				data : {Apdoc : "", Sgubn : '20'}
			});
	...
	preRegCheck : function(oController, vSgubn) {
		var oModel = sap.ui.getCore().getModel("ZHR_BSUIT_SRV"),
		...
		oModel.read("/BsuitApplySet", {
			...
			filters : [
				...
				new sap.ui.model.Filter('Sgubn', sap.ui.model.FilterOperator.EQ, vSgubn)
			],
			success : function(data, res) {
				isValid = true;
			},
	...
	onSelectRow : function(oEvent) {
		...
		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : "ZUI5_HR_Smock.ZUI5_HR_SmockDetail",
			data : { 
				Apdoc : oModel.getProperty(vContext.sPath + "/Apdoc"),
				Sgubn : oModel.getProperty(vContext.sPath + "/Sgubn")
			}
		});
	},
	...
	onPressRow : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Smock.ZUI5_HR_SmockList"),
			oController = oView.getController(),
			vApdoc = oEvent.getSource().getCustomData()[0].getValue(),
			vSgubn = oEvent.getSource().getCustomData()[1].getValue();
				
		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : "ZUI5_HR_Smock.ZUI5_HR_SmockDetail",
			data : {Apdoc : vApdoc, Sgubn : vSgubn} 
		});
	},
	...
	onPressSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Smock.ZUI5_HR_SmockList"),
			oController = oView.getController(),
			oModel = sap.ui.getCore().getModel("ZHR_BSUIT_SRV"),
			oTable = sap.ui.getCore().byId(oController.PAGEID + "_Table"),
		...
		if(SerchCond.Sgubn != "" && SerchCond.Sgubn != "All") {
			aFilters.push(new sap.ui.model.Filter('Sgubn', sap.ui.model.FilterOperator.EQ, SerchCond.Sgubn));
		}
		if(SerchCond.Encid != "") {
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.PantwrW = OneData.PantwrW == '0.0' ? '' : OneData.PantwrW;
...
//			}
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0194") + "-" + dateFormat.format(curDate) + ".xlsx"	// 194:방염 작업복 신청
			};
