...
sap.ui.controller("ZUI5_HR_DeductionFixDisplay.ZUI5_HR_DeductionFixDisplayList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_DeductionFixDisplay.ZUI5_HR_DeductionFixDisplayList
	 */

	PAGEID : "ZUI5_HR_DeductionFixDisplayList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			...
			var JSonData = { Data : { Cpdeymbeg : dateFormat.format(prevDate) , Cpdeymend : dateFormat.format(begDate), Encid : vEncid, Ename : vEname }};
	...
	onPressSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_DeductionFixDisplay.ZUI5_HR_DeductionFixDisplayList"),
			oController = oView.getController(),
			oModel = sap.ui.getCore().getModel("ZHR_DEDUCT_FIX_DISP_SRV"),
		...
		if(SerchCond.Cpdeymbeg != "" && SerchCond.PdeymEnd != "") {
			if(common.Common.checkDate(SerchCond.Cpdeymbeg, SerchCond.PdeymEnd) == false) {
				return ;
			}
		}
		
		if(SerchCond.Cpdeymbeg != "") {
			begDate = sap.ui.getCore().byId(oController.PAGEID + "_Cpdeymbeg").getValue();
			begDate = begDate.split('-');
			begDate = begDate[0] + begDate[1];
			aFilters.push(new sap.ui.model.Filter('Cpdeymbeg', sap.ui.model.FilterOperator.EQ, begDate));
		}
		if(SerchCond.Cpdeymend != "") {
			endDate = sap.ui.getCore().byId(oController.PAGEID + "_Cpdeymend").getValue();
			endDate = endDate.split('-');
			endDate = endDate[0] + endDate[1];
			aFilters.push(new sap.ui.model.Filter('Cpdeymend', sap.ui.model.FilterOperator.EQ, endDate));
		}
		if(SerchCond.Encid != "") {
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							totalSum += Number(OneData.Reqamt);
							
							OneData.Idx = vIdx++;
							OneData.Betrg = common.Common.numberWithCommas(OneData.Betrg);
							OneData.Pdeym = (OneData.Pdeym).replace(/(\d{4})(\d{2})/g, '$1.$2');
						...
						if(totalSum) {
							OneData = {
								Idx : vIdx,
								Perid : oBundleText.getText("LABEL_0142"),	// 142:합계
								Reqamt : common.Common.numberWithCommas(totalSum)
							}
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0589") + "-" + dateFormat.format(curDate) + ".xlsx"	// 589:고정공제 조회(개인)
		};
