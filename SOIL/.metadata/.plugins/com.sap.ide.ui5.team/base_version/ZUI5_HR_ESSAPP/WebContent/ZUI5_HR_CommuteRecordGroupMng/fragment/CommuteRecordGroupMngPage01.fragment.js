...
sap.ui.jsfragment("ZUI5_HR_CommuteRecordGroupMng.fragment.CommuteRecordGroupMngPage01", {
	
	_colModel : [
		{id: "Idx", label : "No.", plabel : "No.", resize : false, span : 0, type : "listText9", sort : true, filter : true, width : "4%"},
	...
	getFilterLayoutRender : function(oController) {
		...
		var aRows = [
			new sap.ui.commons.layout.MatrixLayoutRow({
				...
				cells : [
					...
					new sap.ui.commons.layout.MatrixLayoutCell({
						content : new sap.m.DatePicker(oController.PAGEID + "_Tsmon", {
					...
					new sap.ui.commons.layout.MatrixLayoutCell({
						content : new sap.m.ComboBox(oController.PAGEID + "_Tmare", {
							width : "150px",
							selectedKey : "{Tmare}",
							items : {
					...
					new sap.ui.commons.layout.MatrixLayoutCell({
						content : new sap.m.Input(oController.PAGEID + "_Zename", {
							width : "150px",
							showValueHelp: true,
							valueHelpOnly: false,
							value : "{Zename}",
							customData : new sap.ui.core.CustomData({key : "Zencid", value : "{Zencid}"}),
							change : oController.EmpSearchByTx,
							valueHelpRequest: oController.displayEmpSearchDialog
						}).addStyleClass("Font14px FontColor6")
					}),
			...
			new sap.ui.commons.layout.MatrixLayoutRow({
				...
				cells : [
					...
					new sap.ui.commons.layout.MatrixLayoutCell({
						content : new sap.m.Text({
							text : oBundleText.getText("LABEL_0914")	// 914:개인커멘트 존재여부
						}).addStyleClass("Font14px FontBold")
					}).addStyleClass("PaddingLeft20"),
					new sap.ui.commons.layout.MatrixLayoutCell({
						content : new sap.m.ComboBox({
		...
		return new sap.ui.commons.layout.MatrixLayout({
			...
			columns : 9,
			width : "100%",
			widths : ["100px", "200px", "120px", "200px", "170px","200px", "120px", "200px", ""],
			rows : aRows
		})
...
//			selectionBehavior : sap.ui.table.SelectionBehavior.RowOnly,
			...
			extension : new sap.m.Toolbar({	
				content : [
					...
					new sap.m.Text({
						text : oBundleText.getText("LABEL_0915")	// 915:근태기록부확인
					...
					new sap.m.Button(oController.PAGEID + "_MailBtn", {
						text: oBundleText.getText("LABEL_2903"),	// 2903:메일발송
						type : sap.m.ButtonType.Ghost,
						press : oController.onPressMail,
						visible : false
					})
				]
			}).addStyleClass("ToolbarNoBottomLine")
		...
		oTable.addEventDelegate({
			onAfterRendering: function() {
				...
				common.Common.generateRowspan({
					selector : '#ZUI5_HR_CommuteRecordGroupMngList_Table-header > tbody',
...
//			columnHeaderHeight : 35,
			...
			extension : new sap.m.Toolbar({	
				content : [
					...
					new sap.m.ToolbarSpacer(),
					new sap.m.Button(oController.PAGEID + "_DetailSaveBtn", {
						text: oBundleText.getText("LABEL_0177"),	// 177:저장
						type : sap.m.ButtonType.Ghost,
						press : oController.onDetailSave,
						visible : false
					}),
					new sap.m.Button({
						text: oBundleText.getText("LABEL_2871"),	// 2871:엑셀다운로드
		...
		oTable.getColumns()[17].setTemplate(new sap.m.Input({
			value : "{Cmnta}"
