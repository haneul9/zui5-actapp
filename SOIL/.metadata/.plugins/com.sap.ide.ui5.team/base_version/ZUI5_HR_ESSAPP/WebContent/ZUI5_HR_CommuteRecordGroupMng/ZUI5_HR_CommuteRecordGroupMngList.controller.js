...
sap.ui.controller("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList", {
	/**
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1) {			
			...
			var JSonData = { Data : { Tsmon : dateFormat.format(prevDate), Tmare : "", Encid : "", Ename : "", Zename : "", Zencid : "", Orgeh : "", Orgtx : "", Finyn : "ALL", Rmkyn : "ALL" }};
...
//		this.onPressSearch();
	...
	onSelectRow : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList"),
	...
	onSearchDetailTable : function(oController, vEncid) {
		var oModel = sap.ui.getCore().getModel("ZHR_MONTHLYTIME_SRV"),
			...
			oDetailSaveBtn = sap.ui.getCore().byId(oController.PAGEID + "_DetailSaveBtn"),
		...
		function Search() {
			...
			oDetailSaveBtn.setVisible(Datas.Data.length > 0 ? true : false);
	...
	onPressSearch : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList"),
			...
			oMailBtn = sap.ui.getCore().byId(oController.PAGEID + "_MailBtn"),
			oTableTxt = sap.ui.getCore().byId(oController.PAGEID + "_TableTxt"),
			isVisible = false,
			aConfirmRowIndexs = [];
		...
		oDetailTable.setVisibleRowCount(1);
		oDetailSaveBtn.setVisible(false);
		...
		if(SerchCond.Tmare != "") {
			aFilters.push(new sap.ui.model.Filter('Tmare', sap.ui.model.FilterOperator.EQ, SerchCond.Tmare));
		}
		...
		if(SerchCond.Rmkyn != "" && SerchCond.Rmkyn != "ALL") {
			aFilters.push(new sap.ui.model.Filter('Rmkyn', sap.ui.model.FilterOperator.EQ, SerchCond.Rmkyn));
		}
		if(SerchCond.Encid != "") {
			aFilters.push(new sap.ui.model.Filter('Encid', sap.ui.model.FilterOperator.EQ, SerchCond.Encid));
		}
		if(SerchCond.Zencid != "") {
			aFilters.push(new sap.ui.model.Filter('Zencid', sap.ui.model.FilterOperator.EQ, SerchCond.Zencid));
		}
		if(SerchCond.Orgeh != "") {
			aFilters.push(new sap.ui.model.Filter('Orgeh', sap.ui.model.FilterOperator.EQ, SerchCond.Orgeh));
		}
		aFilters.push(new sap.ui.model.Filter('Termf', sap.ui.model.FilterOperator.EQ, false));
...
//			oController = oView.getController();
	...
	onDetailSave : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList"),
			oController = oView.getController(),
			oTable = sap.ui.getCore().byId(oController.PAGEID + "_DetailTable"),
			oModel = sap.ui.getCore().getModel("ZHR_MONTHLYTIME_SRV"),
			vTsmon = sap.ui.getCore().byId(oController.PAGEID + "_Tsmon").getValue().split("-"),
			vTableData = oController._ListDetailJSonModel.getProperty("/Data"),
			errData = {},
			vOData = {},
			vDetailDataList = [],
			rowObject = {};
		...
		var onProcess = function() {
			...
			vTableData.forEach(function(elem) {
				var convertData = common.Common.copyByMetadata(oModel, "EssMonthlyTimeDetail", elem);
				convertData.Tsdat = "\/Date("+ common.Common.getTime(convertData.Tsdat)+")\/";
				
				vDetailDataList.push(convertData);
			});
			
			vOData.MssMonthlyTimeNav2 = vDetailDataList;
	...
	onPressMail : function(oEvent) {
		...
		if(oController._MailDialog) oController._MailDialog.destroy(); 
		
		oController._MailDialog = sap.ui.jsfragment("ZUI5_HR_CommuteRecordGroupMng.fragment.MailForm", oController);
		oView.addDependent(oController._MailDialog);
		
		oController._DetailMailJSonModel.setData({Data : {}});
		
		oController._MailDialog.open();
	},
	
	beforeOpenMailDialog : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList"),
			oController = oView.getController(),
			oModel = sap.ui.getCore().getModel("ZHR_MONTHLYTIME_SRV"),
			vTsmon = oController._ListCondJSonModel.getProperty("/Data/Tsmon");
		
		if(vTsmon && vTsmon.indexOf("-") > -1) {
			vTsmon = vTsmon.split("-");
			vTsmon = vTsmon[0] + vTsmon[1];
			
			oController._DetailMailJSonModel.setProperty("/Data/Tsmon", vTsmon);
		}
		
		oModel.read("MssMonthlyTimeHeaderSet", {
			async : false,
			filters : [
				new sap.ui.model.Filter('Tsmon', sap.ui.model.FilterOperator.EQ, vTsmon),
			],
			success : function(data, res) {
				if(data.results && data.results.length) {
					oController._DetailMailJSonModel.setData({Data : data.results[0]});
				}
			},
			error : function(Res) {
				var errData = common.Common.parseError(Res);
	...
	onSendMail : function(oController) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_CommuteRecordGroupMng.ZUI5_HR_CommuteRecordGroupMngList"),
			oController = oView.getController(),
			oModel = sap.ui.getCore().getModel("ZHR_MONTHLYTIME_SRV"),
			oTable = sap.ui.getCore().byId(oController.PAGEID + "_Table"),
			aSelectIndices = oTable.getSelectedIndices(),
			vMailHeaderData = oController._DetailMailJSonModel.getProperty("/Data"),
			vDetailDataList = [],
			rowObject = {},
			errData = {};
		
		if(!vMailHeaderData.Title) {
			new control.ZNK_SapBusy.oErrorMessage(oBundleText.getText("LABEL_2907"));	// 2907:제목을 입력하세요.
			return ;
		}
		if(!vMailHeaderData.Content) {
			new control.ZNK_SapBusy.oErrorMessage(oBundleText.getText("LABEL_2908"));	// 2908:본문을 입력하세요.
			return ;
		}
		
		oController.BusyDialog.open();
		
		var mailData = common.Common.copyByMetadata(oModel, "MssMonthlyTimeHeader", vMailHeaderData);
		mailData.Prcty = "S";
		mailData.Actty = _gAuth;
		...
		aSelectIndices.forEach(function(i) {
			...
			vDetailDataList.push({Pernr : rowObject.Pernr});
		});
		
		mailData.MssMonthlyTimeNav2 = vDetailDataList;
		
		oModel.create("/MssMonthlyTimeHeaderSet", mailData, {
			success: function(data,res) {
				if(data) {
					if(data.RetType == "S") {
						sap.m.MessageBox.success(data.RetMsg, {
							title : oBundleText.getText("LABEL_2910")	// 2910:성공
						});
					} else if(data.RetType == "W") {
						sap.m.MessageBox.show(data.RetMsg, {
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0913") + "-" + dateFormat.format(curDate) + ".xlsx"	// 913:개인별근태기록부내역관리
		};
