...
sap.ui.controller("ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageList
	 */

	PAGEID : "ZUI5_HR_ForeignLanguageList",
	...
	onBeforeShow : function(oEvent) {
		var dateFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({pattern: "yyyy-MM-dd"}),
			curDate = new Date(),
			prevDate = new Date(curDate.getFullYear() - 1, curDate.getMonth(), curDate.getDate()+1),
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1){			
			...
			var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , Encid : vEncid, Ename : vEname, ZreqForm : "ALL" }};
			oController._ListCondJSonModel.setData(JSonData);
			
			// 메뉴얼 버튼 활성화
			common.Common.setInformationButton(oController, "A");
		} else {
			var vZreqForm = oController._ListCondJSonModel.getProperty("/Data/ZreqForm") || 'ALL';
			oController._ListCondJSonModel.setProperty("/Data/ZreqForm", vZreqForm);
		}
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageDetail",
		      data : { Appno : _Appno }
		});
	},
	
	retrieveExamCount : function(oController) {
		...
		oModel.read("/LangExCountSet", {
			...
			success : function(data, res) {
				if(data.results && data.results.length) {
					if(Number(data.results[0].Excnt) > 1) {
						sap.m.MessageBox.alert(oBundleText.getText("LABEL_0240"),{});	// 240:당해년도 시험비용 신청이 2회를 초과하였습니다.\n시험 점수만 등록 가능합니다.
					}
				}
			},
			error : function(Res) {}
	...
	onPressNew : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageList"),
			oController = oView.getController(),
			vZreqForm = oEvent.getSource().getCustomData()[0].getValue(),
			detailForm = (vZreqForm == 'BE01') 
							? "ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageLesson"
							: "ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageExam";
		
		// 시험횟수 조회
		if(vZreqForm == 'BE02') oController.retrieveExamCount(oController);
	...
	onSelectRow : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageList"),
			...
			vAppno = rowObject.Appno,
			vZreqForm = rowObject.ZreqForm,
			detailForm = (vZreqForm == 'BE01') 
							? "ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageLesson"
							: "ZUI5_HR_ForeignLanguage.ZUI5_HR_ForeignLanguageExam";
		
		if(common.Common.isNull(vContext) || rowObject == null) {
			return;
		}
	...
	onPressSearch : function(oEvent) {
		...
		if(SerchCond.ZreqForm != "" && SerchCond.ZreqForm != "ALL") {
				...
				success: function(data,res){
					if(data && data.results.length){
						for(var i=0;i<data.results.length;i++){
							...
							OneData.Idx = i + 1;
							OneData.Appamt = common.Common.numberWithCommas(OneData.Appamt);
							OneData.Field1 = (OneData.ZreqForm == "BE01") ? OneData.Eduorg : OneData.Exmtytx;
							OneData.Field2 = (OneData.ZreqForm == "BE01") ? OneData.Edutim : OneData.Score;
							OneData.Field3 = (OneData.ZreqForm == "BE01") ? OneData.Eduwaytx : OneData.Gradetx;
...
//			}
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0260") + "-" + dateFormat.format(curDate) + ".xlsx"	// 260:외국어 학습비/시험 비용 신청
			};
