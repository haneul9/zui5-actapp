...
sap.ui.controller("ZUI5_HR_Deduction.ZUI5_HR_DeductionList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_Deduction.ZUI5_HR_DeductionList
	 */

	PAGEID : "ZUI5_HR_DeductionList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_Deduction.ZUI5_HR_DeductionList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : "ZUI5_HR_Deduction.ZUI5_HR_DeductionDetail",
			data : { Appno : _Appno }
		});
	},
	...
	onPressSearch : function(oEvent) {
		...
		if(SerchCond.Reqymbeg != "") {
			vReqymbeg = sap.ui.getCore().byId(oController.PAGEID + "_Reqymbeg").getValue();
			vReqymbeg = vReqymbeg.split('-');
			vReqymbeg = vReqymbeg[0] + vReqymbeg[1];
			aFilters.push(new sap.ui.model.Filter('Reqymbeg', sap.ui.model.FilterOperator.EQ, vReqymbeg));
		}
		if(SerchCond.Reqymend != "") {
			vReqymend = sap.ui.getCore().byId(oController.PAGEID + "_Reqymend").getValue();
			vReqymend = vReqymend.split('-');
			vReqymend = vReqymend[0] + vReqymend[1];
			aFilters.push(new sap.ui.model.Filter('Reqymend', sap.ui.model.FilterOperator.EQ, vReqymend));
		}
		if(SerchCond.Dedcd != "") {
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.Reqamt = common.Common.numberWithCommas(OneData.Reqamt);	// 공제금액
							OneData.Reqym = (!OneData.Reqym || OneData.Reqym == "000000") ? "" : OneData.Reqym.replace(/(\d{4})(\d{2})/g, '$1.$2');
							OneData.ZreqDatetime = (OneData.ZreqDate) ? dateFormat.format(new Date(common.Common.setTime(OneData.ZreqDate))) : "";
							OneData.ZreqDatetime = (OneData.ZreqDatetime && OneData.ZreqTime) ? OneData.ZreqDatetime + " " + common.Common.timeToString(OneData.ZreqTime, "HH:mm:ss") : OneData.ZreqDatetime;
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0485") + "-" + dateFormat.format(curDate) + ".xlsx"	// 485:[공장]고정공제 신청
		};
