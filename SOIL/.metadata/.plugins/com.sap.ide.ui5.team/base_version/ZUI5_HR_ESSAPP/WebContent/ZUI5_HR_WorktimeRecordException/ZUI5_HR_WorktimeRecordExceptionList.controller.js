...
sap.ui.controller("ZUI5_HR_WorktimeRecordException.ZUI5_HR_WorktimeRecordExceptionList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_WorktimeRecordException.ZUI5_HR_WorktimeRecordExceptionList
	 */

	PAGEID : "ZUI5_HR_WorktimeRecordExceptionList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_WorktimeRecordException.ZUI5_HR_WorktimeRecordExceptionList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
			id : "ZUI5_HR_WorktimeRecordException.ZUI5_HR_WorktimeRecordExceptionDetail",
			data : { Appno : _Appno }
		});
	},
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.Tsmon = (!OneData.Tsmon || OneData.Tsmon == "000000") ? "" : OneData.Tsmon.replace(/(\d{4})(\d{2})/g, '$1.$2');	// 귀속월
...
//			oController = oView.getController();
	...
	onExport : function(oEvent){
		...
		var oSettings = {
				...
				fileName: oBundleText.getText("LABEL_0802") + "-" + dateFormat.format(curDate) + ".xlsx"	// 802:근태기록부(예외자)
		};
