...
sap.ui.controller("ZUI5_HR_PreparedAmount.ZUI5_HR_PreparedAmountList", {
	/**
	...
	PAGEID : "ZUI5_HR_PreparedAmountList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_PreparedAmount.ZUI5_HR_PreparedAmountList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_PreparedAmount.ZUI5_HR_PreparedAmountDetail",
		      data : { Appno : _Appno }
		});
	},
	...
	onPressSearch : function(oEvent) {
		...
		if(SerchCond.Payym != "") {
			aFilters.push(new sap.ui.model.Filter('Payym', sap.ui.model.FilterOperator.EQ, sap.ui.getCore().byId(oController.PAGEID + "_Payym").getValue()));
		}
		if(SerchCond.Apbeg != "") {
		...
		function Search() {
			...
			oModel.read("/BefMovingRequestApplSet", {
				async: false,
				filters: aFilters,
...
//		control.ZNKBusyAccessor.onBusy(oEvent, oController);
	...
	onExport : function(oEvent) {
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_1372") + "-" + dateFormat.format(curDate) + ".xlsx"	// 1372:부임/귀임 준비금 신청
			};
