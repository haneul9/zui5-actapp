...
sap.ui.controller("ZUI5_HR_Volunteer.ZUI5_HR_VolunteerList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_Volunteer.ZUI5_HR_VolunteerList
	 */

	PAGEID : "ZUI5_HR_VolunteerList",
	...
	onBeforeShow : function(oEvent) {
		...
		if(!oController._ListCondJSonModel.getProperty("/Data") || oController._ListCondJSonModel.getProperty("/Data").length < 1){			
			...
			var JSonData = { Data : { Apbeg : dateFormat.format(prevDate) , Apend : dateFormat.format(curDate) , 
				                      Pernr : vPernr, Ename : vEname, Encid : vEncid }};
	...
	onPressNew1 : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Volunteer.ZUI5_HR_VolunteerList");
		var oController = oView.getController();
		oController._vPType = "GA01";
		oController._vPSelectedIndx = "";
		oController.onPressRegCheck();
	},
	
	onPressNew2 : function(oEvent){
		var oView = sap.ui.getCore().byId("ZUI5_HR_Volunteer.ZUI5_HR_VolunteerList");
		var oController = oView.getController(),
			vData = oController._ListJSonModel.getProperty("/Data");
		var vCount = 0;
		for(var i = 0; i < vData.length; i++){
			if(vData[i].Check == true){
				vCount++;
				oController._vPPland = vData[i].Appno;
			}	
		}
		
		if(vCount == 0){
			sap.m.MessageBox.alert(oBundleText.getText("LABEL_2500"), {title : oBundleText.getText("LABEL_0053")});	// 2500:사회봉사 계획서를 선택하여 주시기 바랍니다.
			return ;
		}else if(vCount > 1){
			sap.m.MessageBox.alert(oBundleText.getText("LABEL_2631"), {title : oBundleText.getText("LABEL_0053")});	// 2631:하나의 사회봉사 계획서만 선택하여 주시기 바랍니다.
			oController._vPPland = "";
			return;
		}
		
		oController._vPType = "GA02";
		oController.onPressRegCheck();
	},
	...
	onPressNew : function(oEvent) {
		...
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_Volunteer.ZUI5_HR_VolunteerDetail",
		      data : { Appno : "" , ZreqForm : oController._vPType, Pland : oController._vPPland }
		});
	},
	...
	onPressRow : function(oEvent) {
		...
		oController = oView.getController(),		
		oTable = sap.ui.getCore().byId(oController.PAGEID+"_Table"),
		vContext = oEvent.getSource().getBindingContext().sPath,
		oModel = oTable.getModel();
	
		if(common.Common.isNull(vContext)){
			return;
		}
	
		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_Volunteer.ZUI5_HR_VolunteerDetail",
		      data : { Appno : oModel.getProperty(vContext + "/Appno"),
		    	       ZreqForm : oModel.getProperty(vContext + "/ZreqForm")
		      }
	...
	onPressSearch : function(oEvent) {
		...
		var aFilters = [],
			SearchCond = oController._ListCondJSonModel.getProperty("/Data");
		
		if(SearchCond.Apbeg != "" && SearchCond.Apend != "") {
		...
		if(SearchCond.Encid != "") {
			aFilters.push(new sap.ui.model.Filter('Encid', sap.ui.model.FilterOperator.EQ, SearchCond.Encid));
		}
		
		if(common.Common.checkNull(SearchCond.Key)) {
			SearchCond.Key = "A";
		}
		
		aFilters.push(new sap.ui.model.Filter('ZreqForm', sap.ui.model.FilterOperator.EQ, SearchCond.Key));
...
//			}
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_1835") + "-" + dateFormat.format(curDate) + ".xlsx"	// 1835:사회봉사 계획/보고서
			};
