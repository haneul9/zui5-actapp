...
sap.ui.controller("ZUI5_HR_HousingFundRepayment.ZUI5_HR_HousingFundRepaymentList", {
	/**
	...
	PAGEID : "ZUI5_HR_HousingFundRepaymentList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_HousingFundRepayment.ZUI5_HR_HousingFundRepaymentList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_HousingFundRepayment.ZUI5_HR_HousingFundRepaymentDetail",
		      data : { Appno : _Appno }
		});
	},
	
	prevCheckNew : function(oController) {
	...
	onPressNew : function(oEvent) {
		...
		if(oController.prevCheckNew(oController)) {
			sap.ui.getCore().getEventBus().publish("nav", "to", {
				id : "ZUI5_HR_HousingFundRepayment.ZUI5_HR_HousingFundRepaymentDetail",
				data : {Appno : ""}
			});
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.LnamtSum = common.Common.numberWithCommas(OneData.LnamtSum);
							OneData.MrpprSum = common.Common.numberWithCommas(OneData.MrpprSum);
...
//			}
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0420") + "-" + dateFormat.format(curDate) + ".xlsx"	// 420:주택자금 중도상환 신청
			};
