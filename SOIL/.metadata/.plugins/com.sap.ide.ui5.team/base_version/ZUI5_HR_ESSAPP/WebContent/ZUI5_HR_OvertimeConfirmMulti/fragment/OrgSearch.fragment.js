sap.ui.jsfragment("ZUI5_HR_OvertimeConfirmMulti.fragment.OrgSearch", {

	createContent : function(oController) {	
		
		jQuery.sap.require("ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch");
		...
		var oFilterBar = new sap.m.Toolbar({
			content : [ 
				...
				new sap.m.Input(oController.PAGEID + "_OrgSearch_Stext", {
					width: "150px"
				}).attachBrowserEvent("keyup", ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onKeyUp),
				new sap.m.ToolbarSpacer({width: "10px"}),
				new sap.m.Button(oController.PAGEID + "_OrgSearch_SearchButton", {
					...
					press : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.OrgSearch
				}),
				new sap.m.ToolbarSpacer({width: "10px"})
			]
		...
		var oTable = new sap.m.Table(oController.PAGEID + "_OrgSearch_TABLE", {
			...
			columns : [
				new sap.m.Column({
					header: new sap.m.Label({text : oBundleText.getText("LABEL_0039"), textAlign : "Center"}).addStyleClass("L2PFontFamily"),	// 39:소속부서
					demandPopin: true,
				...
				new sap.m.Column({
					header: new sap.m.Label({text : oBundleText.getText("LABEL_0531") , textAlign : "Center" }).addStyleClass("L2PFontFamily"),	// 531:조직장
					demandPopin: true,
		...
		var oDialog = new sap.m.Dialog(oController.PAGEID + "_OrgSearch_Dialog", {
			...
			showHeader : true,
			title : "{Title}",
			beforeOpen : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onBeforeOpenOrgSearchDialog,
			afterOpen : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onAfterOpenOrgSearchDialog,
			afterClose : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onAfterCloseOrgSearchDialog,
			beginButton :  new sap.m.Button({
				icon : "sap-icon://accept",
				text : oBundleText.getText("LABEL_0037"),	// 37:선택
				press : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onConfirm}),
			endButton : new sap.m.Button({
				icon : "sap-icon://sys-cancel-2",
				text :  oBundleText.getText("LABEL_0071"),	// 71:취소
				press : ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onClose}),
			content : [oLayout]
		}).setModel(new sap.ui.model.json.JSONModel())	
		.bindElement("/Data");
