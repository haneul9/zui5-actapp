jQuery.sap.declare("ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch");
...
ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch = {
	/** 
	* @memberOf ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch
	*/	
...
//			oTable.bindItems("/OrgListSet");
	...
	onKeyUp : function(oEvent) {
		if(oEvent.which == 13) {
			ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.OrgSearch();
		}
	},
	...
	onConfirm : function(oEvent) {
		...
		var oWorktimeDialog = oInput.getParent().getParent().getParent();
		...
		if(oInput) {
			if(vSelectedOrg && vSelectedOrg.length) {
				...
				oWorktimeDialog.getModel().setProperty("/Data/Orgeh", vSelectedOrg[0].Orgeh); 
				oWorktimeDialog.getModel().setProperty("/Data/Orgtx", vSelectedOrg[0].Stext); 
			}
		}
	...
	onClick : function(oEvent) {
		...
		ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clicks = ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clicks + 1;
		
		if(ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clicks == 1) {
			setTimeout( ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clearClicks, 500);
		} else if(ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clicks == 2) {
			ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch._vSPath = vContext.sPath;
		}
	},
	...
	clearClicks : function () {
		var oView = sap.ui.getCore().byId("ZUI5_HR_OvertimeConfirmMulti.ZUI5_HR_OvertimeConfirmMultiDetail");
		var oController = oView.getController();
		ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.clicks = 0;
		ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch._vSPath = -1;
	...
	onDblClick : function(oEvent){
		...
		var oController = oView.getController();
		var oTable = sap.ui.getCore().byId(oController.PAGEID + "_OrgSearch_TABLE");
		if(ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch._vSPath == null || ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch._vSPath == -1) return ;
		var vIndex = ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch._vSPath.replace("/OrgListSet/","");
		oTable.clearSelection();
		oTable.setSelectedIndex(parseInt(vIndex));
		ZUI5_HR_OvertimeConfirmMulti.control.OrgSearch.onConfirm();
	},
