...
sap.ui.controller("ZUI5_HR_LifeStableRepayment.ZUI5_HR_LifeStableRepaymentList", {
	/**
	 ...
	 * 
	 * @memberOf ZUI5_HR_LifeStableRepayment.ZUI5_HR_LifeStableRepaymentList
	 */

	PAGEID : "ZUI5_HR_LifeStableRepaymentList",
	...
	onDetailPage : function(oEvent) {
		var oView = sap.ui.getCore().byId("ZUI5_HR_LifeStableRepayment.ZUI5_HR_LifeStableRepaymentList"),
			oController = oView.getController();

		sap.ui.getCore().getEventBus().publish("nav", "to", {
	    	  id : "ZUI5_HR_LifeStableRepayment.ZUI5_HR_LifeStableRepaymentDetail",
		      data : { Appno : _Appno }
		});
	},
	...
	onPressNew : function(oEvent) {
		...
		if(oController.checkPrevNew(oController)) {
			sap.ui.getCore().getEventBus().publish("nav", "to", {
				id : "ZUI5_HR_LifeStableRepayment.ZUI5_HR_LifeStableRepaymentDetail",
				data : {Appno : ""}
			});
	...
	checkPrevNew : function(oController) {
		...
		oModel.read("/LifeStableLoanRepaySet", {
			...
			filters : [
				...
				new sap.ui.model.Filter('Encid', sap.ui.model.FilterOperator.EQ, vEmpLoginInfo[0].Encid)
			],
				...
				success: function(data,res) {
					if(data && data.results.length) {
						for(var i=0;i<data.results.length;i++) {
							...
							OneData.Idx = i + 1;
							OneData.Lnamt = common.Common.numberWithCommas(OneData.Lnamt);
							OneData.LmamtPri = common.Common.numberWithCommas(OneData.LmamtPri);
							OneData.LmamtSum = common.Common.numberWithCommas(OneData.LmamtSum);
...
//			}
	...
	onExport : function(oEvent){
		...
		var oSettings = {
			    ...
			    fileName: oBundleText.getText("LABEL_0431") + "-" + dateFormat.format(curDate) + ".xlsx"	// 431:생활안정자금 중도상환 신청
			};
