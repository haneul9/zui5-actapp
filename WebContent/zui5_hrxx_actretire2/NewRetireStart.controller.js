...
sap.ui.controller("zui5_hrxx_actretire2.NewRetireStart", {
...
// KYJ
		...
		this._vViewType = this.onSearchViewtype();
		console.log(this._vViewType);
		this.onSetLayout();
//		this.displaySearchUserDialog();
		oPernr.fireValueHelpRequest();
	},
...
//	}
	/*
	 * 인사영역에 해당하는 Viewtype -> ISO 국가코드를 조회
	 */
	
	onSearchViewtype : function(oEvent){
		/*
		 *  1: US
			2: UK
			3: CZ
            9: XX
		 */
		var oView = sap.ui.getCore().byId("zui5_hrxx_actretire2.NewRetireStart");
		var oController = oView.getController();
		var vRetData = "";
		var oModel = sap.ui.getCore().getModel("ZHRXX_RETAPPL_SRV");
		oModel.read("/TerminationProcessTypeSet/?$filter=Persa%20eq%20%27" + oController._vPersa + "%27", 
				null, 
				null, 
				false,
				function(oData, oResponse) {					
					if(oData && oData.results.length) {
						vRetData = oData.results[0].Termp;
...
//		oController._vViewType = oController.onSearchViewtype();
//		oController.onSetLayout();
	},
	...
	displayCodeSearchDialog : function(oEvent) {
		...
		if(oController._vPersa == ""){
			common.Common.showErrorMessage(oBundleText.getText(""));
			return ;
		}
