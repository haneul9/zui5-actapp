sap.ui.jsview("zui5_hrxx_actapp2.ActRecDocument", {
	...
	*/ 
	getControllerName : function() {
		return "zui5_hrxx_actapp2.ActRecDocument";
	},
	...
	createContent : function(oController) {
		...
		oCell = new sap.ui.commons.layout.MatrixLayoutCell({
			...
			content : new sap.m.Select(oController.PAGEID + "_Persa", {
					...
					}).addStyleClass("L2P13Font") //.setModel(sap.ui.getCore().getModel("ZL2P01GW0001_SRV"))
		...
		oCell = new sap.ui.commons.layout.MatrixLayoutCell({
			hAlign : sap.ui.commons.layout.HAlign.Begin,
			vAlign : sap.ui.commons.layout.VAlign.Middle,
			content : [new sap.m.Label({text: oBundleText.getText("PVDOC")}).addStyleClass("L2P13Font")]
		}).addStyleClass("L2PInputTableLabel L2PPaddingLeft10");
		oRow.addCell(oCell);
		
		var oPvdoc = new sap.m.CheckBox(oController.PAGEID + "_Pvdoc", {
			text : oBundleText.getText("PVDOC_COMMENT"),
			selected : true
		}).addStyleClass("L2P13Font");
		...
		var oSyncToolbar = new sap.m.Toolbar(oController.PAGEID + "_SyncToolbar", {
			...
			content : [new sap.m.ToolbarSpacer(),
			           new sap.m.Button(oController.PAGEID + "_Sync_Rec_Btn",{text: oBundleText.getText("SYNC_REC_BTN"), icon : "sap-icon://synchronize", press : oController.syncRecPerson, visible : false, type : "Default"})]
		}).addStyleClass("L2PToolbarNoBottomLine");
...
//				           new sap.m.Label({text : oBundleText.getText("VERIFY")}).addStyleClass("L2P13Font"),
				           new sap.m.ToolbarSpacer(),
				           new sap.m.Button(oController.PAGEID + "_Add_Rec_Btn",{text: oBundleText.getText("ADD_REC_BTN"), icon : "sap-icon://add-contact", press : oController.addRecPerson, visible : false}),
				           new sap.m.Button(oController.PAGEID + "_Mod_Rec_Btn",{text: oBundleText.getText("MODIFY_REC_BTN"), icon : "sap-icon://edit", press : oController.modifyRecPerson, visible : false}),
				           ...
				           new sap.m.Button(oController.PAGEID + "_Add_Btn",{text: oBundleText.getText("ADD_PER_BTN"), icon : "sap-icon://add", press : oController.addPerson, visible : false}),
				           new sap.m.Button(oController.PAGEID + "_Mod_Btn",{text: oBundleText.getText("MODIFY_PER_BTN"), icon : "sap-icon://edit", press : oController.modifyPerson, visible : false}),
				           new sap.m.Button(oController.PAGEID + "_Del_Btn",{text: oBundleText.getText("DELETE_PER_BTN"), icon : "sap-icon://delete", press : oController.deletePerson, visible : false})]
			}).addStyleClass("L2PToolbarNoBottomLine"),
			content : [oTable]
		});
...
//		 	                new sap.m.Button(oController.PAGEID + "_RECACT_BTN", {
//								 text : oBundleText.getText( "NEW_RECACT_BTN"),
//								 press : oController.onOpenRecActionDialog
//		 	                }),
		...
		var oPage = new sap.m.Page(oController.PAGEID + "_PAGE", {
			...
			customHeader : new sap.m.Bar({
								...
								contentMiddle : new sap.m.Text({
									   			text : oBundleText.getText( "TITLE_RECACTREQ_REG")
