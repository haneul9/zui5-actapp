	...
	onPressComplete : function(oEvent) {
		...
		var onCompleteAction = function() {
			...
			if(process_result) {
				...
				mSubjectList.setProperty("/ActionSubjectListSet/" + d_idx + "/ProcessStatusText", oBundleText.getText("ACTION_COMPLETE_STATUS2"));
				mSubjectList.setProperty("/ActionSubjectListSet/" + d_idx + "/ProcessStatus", "S");
			} else {
				mSubjectList.setProperty("/ActionSubjectListSet/" + d_idx + "/ProcessStatusText", oBundleText.getText("ACTION_COMPLETE_STATUS3"));
				mSubjectList.setProperty("/ActionSubjectListSet/" + d_idx + "/ProcessStatus", "F");
				oController._vFailCnt++;
			}
			
			if((idx+1) < oController._vTotalCnt) {
				mSubjectList.setProperty("/ActionSubjectListSet/" + check_idxs[idx+1] + "/ProcessStatusText", oBundleText.getText("ACTION_COMPLETE_STATUS1"));
...
//					oController._oCompleteEMailDialog.open();
		...
		var onProcessing = function(oAction){
			if ( oAction === sap.m.MessageBox.Action.YES ) {
				...
				mSubjectList.setProperty("/ActionSubjectListSet/0/ProcessStatusText", oBundleText.getText("ACTION_COMPLETE_STATUS1"));
		...
		var vMsg = vMsg1.replace("%CNT%", check_idxs.length) + " " + oBundleText.getText("MSG_COMPLETE_QUESTION");
...
//			else vLang = "en";
	...
	setSubjectListColumn : function(oController) {
		...
		var vColumns = [ {id : "Pchk", label : "", control : "CheckBox", width : 30, align : "Center"},
		                 ...
		                 {id : "Batyp", label : oBundleText.getText("BATYP"), control : "Html", width : 70, align : "Center"},
...
//        		  + "%20and%20Reqno%20eq%20%27" + encodeURI(oController._vReqno) + "%27";
		...
		// 지원자 수
		var vDiffCnt = 0 ;
		if(vHireActionFlag == "X" && vHireVoltId != ""){
			var oSFModel = sap.ui.getCore().getModel("ZSFXX_ONBAPP_SRV"); 
			var vFilterString = "/?$filter=Docno%20eq%20%27" + oController._vDocno + "%27";
			vFilterString += "%20and%20";
			vFilterString += "VoltIdList%20eq%20%27" + vHireVoltId + "%27";
			var vCheckVerifStatusList = [];
			
			try {
				oSFModel.read("/CheckVerifStatusSet" + vFilterString, 
						null, 
						null, 
						false, 
						function(oData, oResponse) {					
							if(oData.results && oData.results.length) {
								for(var i=0; i<oData.results.length; i++) {
									vCheckVerifStatusList.push(oData.results[i]);
								}
							}
						},
						function(oResponse) {
			...
			for(var i=0; i< vActionSubjectList.ActionSubjectListSet.length; i++){
				var existFlag = "";
				for(var j=0; j < vCheckVerifStatusList.length; j++){
					if(existFlag == "X") break;
					if(vActionSubjectList.ActionSubjectListSet[i].VoltId == vCheckVerifStatusList[j].VoltId) existFlag = "X";
				}
				if(existFlag == ""){
					vActionSubjectList.ActionSubjectListSet.splice(i,1);
					SubjectListData.data.splice(i,1);
				//	i--; // 배열이 삭제 되면 index가 자동정렬.
				}
			}
			// 지원자 수
			vDiffCnt = oController._vActRecCount - SubjectListData.data.length ;
		}
