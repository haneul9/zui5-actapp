...
//	}
	onBeforeShow: function(oEvent) {
		...
		//신규작성인 경우
		if(this._vStatu == "00") {
			oPageTitle.setText(oBundleText.getText("TITLE_RETIRE_ACTREQ_REG"));
...
//		}
	...
	onAfterRenderingTable : function(oController) {
		if(typeof ActRetireDocumentSubject == "undefined") {
			var vLang = "";
...
//			else vLang = "en";
			...
			createIBSheet2(document.getElementById(oController.PAGEID + "_SubjectList"), "ActRetireDocumentSubject", "100%",  (window.innerHeight - 360) + "px", vLang);
		}
...
//			else vLang = "en";
		...
		ActRetireDocumentSubject.SetTheme("DS","ActApp");
		...
		try {
			IBS_InitSheet(ActRetireDocumentSubject, initdata);
		}catch(ex) {
			console.log(ex);
		}		
		
		if(vTotalWidth < window.innerWidth) {
			//ActRetireDocumentSubject.FitColWidth();
		}			
		ActRetireDocumentSubject.SetSelectionMode(0);
		
		ActRetireDocumentSubject.SetCellFont("FontSize", 0, "Pchk", ActRetireDocumentSubject.HeaderRows(), "vDummy", 13);
		ActRetireDocumentSubject.SetCellFont("FontName", 0, "Pchk", ActRetireDocumentSubject.HeaderRows(), "vDummy", "Malgun Gothic");
		
		ActRetireDocumentSubject.SetHeaderRowHeight(32);
		ActRetireDocumentSubject.SetDataRowHeight(32);
		
		ActRetireDocumentSubject.SetFocusAfterProcess(0);
		
		var SubjectListData = {data : []};
		
		if(oController._vStatu == "00") {		
			ActRetireDocumentSubject.LoadSearchData(SubjectListData);
			return;
		}
