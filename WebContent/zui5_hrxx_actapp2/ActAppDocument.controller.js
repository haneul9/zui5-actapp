					...
					if(oData.results && oData.results.length) {
						...
						for(var i=0; i<oData.results.length; i++) {
							...
							oneDataSheet.Ename_Html = "<table cellpadding=0 cellspacing=0 border=0 style='background-color:transparent'>"
							                        ...
							                        + "<div id='" + oController.PAGEID + "_row_name_" + i + "' class='L2P13Font' style='color:blue; cursor:pointer;' onclick='onInfoViewPopup(" + i + ")'>" + oData.results[i].Ename + "</div>"
							...
							if(vBatyp == "A") {
								oneDataSheet.Batyp_Html = "<span class='L2P13Font' style='font-weight:bold; color:#1F4E79'>" + oBundleText.getText("AFTER") + "</span>";
								oneDataSheet.Batyp = "AFTER";
							} else {
								oneDataSheet.Batyp_Html = "<span class='L2P13Font' style='font-weight:bold; color:black'>" + oBundleText.getText("BEFORE") + "</span>";
...
var onInfoViewPopup = function(rowId) {
	var oView = sap.ui.getCore().byId("zui5_hrxx_actapp2.ActAppDocument");
	var oController = oView.getController();
	...
	oController._vSelected_Pernr = oModel.getProperty("/data/" + rowId + "/Pernr");
	oController._vSelected_Reqno = oModel.getProperty("/data/" + rowId + "/Reqno"); 
	oController._vSelected_Actda = oModel.getProperty("/data/" + rowId + "/Actda");
	oController._vSelected_Docno =  oModel.getProperty("/data/" + rowId + "/Docno");
	oController._vSelected_VoltId = oModel.getProperty("/data/" + rowId + "/VoltId");
