...
//				}
	...
	modifyPerson : function(oEvent) {
	    ...
	    var cxt = oListTable.getContextByIndex(vContext[0]);
	    var path = cxt.sPath;
		...
		var onProcessDelete = function(fVal) {
			if(fVal && fVal == "OK") {
				...
				for(var i=0; i<vContext.length; i++) {
				    var cxt = oListTable.getContextByIndex(vContext[i]);
				    var path = cxt.sPath;
				    var vOneRecord = oListTable.getModel().getProperty(path);
...
//					oController.setRecActVisible(oController);
	...
	setRecSubjectListColumn : function(oController) {
		...
		vColumns.push({id : "Actda1", label : oBundleText.getText("ACTDA"), control : "Text", width : "120", align: "Center"});
		...
		vColumns.push({id : "Sub08_Img", data : "Sub08", label : oBundleText.getText("TSUB08"), control : "Img", width : "120", align: "Center"});
