...
//			oController.initInputControl(oController);
	...
	getActionInputFieldSet : function(oController, Massn, Massg) {
		...
		filterString += "%20and%20Massg%20eq%20%27" + Massg + "%27";
	...
	makeTable : function(oController, ActiveControl, oLayout, Grouping) {
		...
		for(var i=0; i<vActionSubjectTableHeaderSet.length; i++) {
			...
			var oColumn = new sap.m.Column({
				header: new sap.m.Label({text : vActionSubjectTableHeaderSet[i].Label, required : fRequired}).addStyleClass("L2P13Font"), 			        	  
				demandPopin: true,
...
//				var vAddFilter = [{key : "Persa", value : oController._vPersa},
//				                  {key : "Actda", value : oController._vActda}];			
//				var mEmpCodeList = oController.setSpecialCodeData(Fieldname, vAddFilter, true);				
//				oControl.setModel(mEmpCodeList);
...
//				oControl.bindItems("/EmpCodeListSet", new sap.ui.core.Item({key : "{Ecode}", text : "{Etext}"}), null, []);
				...
				var vExcod = oController._vMolga + "|" + oController._vSelectedTrfar + "|" + oController._vSelectedTrfgb + "|" + oController._vSelectedPersg + "|" + oController._vSelectedPersk;
				var vAddFilter = [{key : "Persa", value : oController._vPersa},
				                  {key : "Actda", value : oController._vActda},
				                  {key : "Excod", value : vExcod}];
				var mDataModel = oController.setSpecialCodeData(Fieldname, vAddFilter, true);	
...
//				}
	...
	setSpecialCodeData : function(Fieldname, vAddFilter, select) {
		...
		if(select == false) {
			vCodeModel.EmpCodeListSet.push({Field : Fieldname, Ecode : "", Etext : oBundleText.getText("NOVALUE")});
		} else {
			vCodeModel.EmpCodeListSet.push({Field : Fieldname, Ecode : "0000", Etext : oBundleText.getText("SELECTDATA")});
		}		
...
//			oTrfgr.removeAllItems();
			...
			if(oControl.getSelectedKey() != "0000") {
				...
				oTrfgr.addItem(new sap.ui.core.Item({key : "0000", text : oBundleText.getText("SELECTDATA")}));
			}
...
//			oTrfst.removeAllItems();
	...
	onPressPersg : function(oEvent) {		
		...
		try {			
			...
			if(oSchkz) {				
				oSchkz.destroyItems();
				oSchkz.addItem(new sap.ui.core.Item({key : "0000", text : oBundleText.getText("SELECTDATA")}));
			}
...
//				oSchkz.setSelectedKey("0000");
		...
		if(oTrfgr && oPersg && oTrfgb && oTrfar) {
			...
			if(oControl.getSelectedKey() != "0000") {
				var vExcod = oController._vMolga + "|" + oTrfar.getSelectedKey() + "|" + oTrfgb.getSelectedKey() + "|" + oPersg.getSelectedKey()  + "|" + oControl.getSelectedKey();
				var vAddFilter = [{key : "Persa", value : oController._vPersa},
				                  {key : "Actda", value : oController._vActda},
				                  {key : "Pernr", value : oController._vPernr},
				                  {key : "Excod", value : vExcod}];
