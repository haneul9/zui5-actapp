...
//		common.GoogleMap.setMap(vBeforeData.Gps, vBeforeData.Zzempwp_Tx, vAfterData.Gps, vAfterData.Zzempwp_Tx);
		...
		var vMassLabels = [oBundleText.getText("ACT_TYPE_REASON_1"), oBundleText.getText("ACT_TYPE_REASON_2"),
		                   oBundleText.getText("ACT_TYPE_REASON_3"), oBundleText.getText("ACT_TYPE_REASON_4"),
		                   oBundleText.getText("ACT_TYPE_REASON_5"),];
		...
		for(var i=0; i<vDisplayControl.length; i++) {
			...
			oCell = new sap.ui.commons.layout.MatrixLayoutCell({
				hAlign : sap.ui.commons.layout.HAlign.Begin,
				vAlign : sap.ui.commons.layout.VAlign.Middle,
				content : [new sap.m.Label({text : vHeaderText}).addStyleClass("L2P13Font")]
			}).addStyleClass("L2PInputTableLabel L2PPaddingLeft10");		
			oRow.addCell(oCell);
...
//		common.GoogleMap.setMap(vAfterData.Gps, vAfterData.Zzempwp_Tx, "", "");
		...
		var vMassLabels = [oBundleText.getText("ACT_TYPE_REASON")]; //["1st 발령유형 / 사유","2nd 발령유형 / 사유","3rd 발령유형 / 사유","4th 발령유형 / 사유","5th 발령유형 / 사유",];
		...
		for(var i=0; i<vDisplayControl.length; i++) {
			...
			var oLabel = new sap.m.Label({text : vHeaderText}).addStyleClass("L2P13Font");
	...
	setSubjectListColumn : function(oController) {
		...
		var icon1 = "<img src='/sap/bc/ui5_ui5/sap/ZL2P01UI59000/images/action1.png'>";
		var icon2 = "<img src='/sap/bc/ui5_ui5/sap/ZL2P01UI59000/images/action2.png'>";
		var icon3 = "<img src='/sap/bc/ui5_ui5/sap/ZL2P01UI59000/images/action3.png'>";
		...
		if(zui5_hrxx_actapp2.common.Common.oSubjectList) {
			var vColNum = zui5_hrxx_actapp2.common.Common.oSubjectList.getColumnsNum();
			for(var i=1; i<vColNum; i++) {
				zui5_hrxx_actapp2.common.Common.oSubjectList.deleteColumn(1);
			}
			zui5_hrxx_actapp2.common.Common.oSubjectList.clearAll(false);
		}
		...
		for(var i=0; i<vColumns.length; i++) {
			zui5_hrxx_actapp2.common.Common.oSubjectList.insertColumn(c_idx, vColumns[i].label, vColumns[i].control, vColumns[i].width, 'na', vColumns[i].align,'middle');
			c_idx++;			
		}
		...
		for(var i=0; i<vDisplayControl.length; i++) {
			...
			zui5_hrxx_actapp2.common.Common.oSubjectList.insertColumn(c_idx, oBundleText.getText(vDisplayControl[i].Fieldname),'ro', "150", 'na','left','middle');
			c_idx++;
		}
...
//        		  + "%20and%20Reqno%20eq%20%27" + encodeURI(oController._vReqno) + "%27";
		...
		zui5_hrxx_actapp2.common.Common.oSubjectList.startFastOperations();
					...
					if(oData.results && oData.results.length) {
						...
						for(var i=0; i<oData.results.length; i++) {
							...
							if(zui5_hrxx_actapp2.common.Common.oSubjectList) {
								...
								zui5_hrxx_actapp2.common.Common.oSubjectList.addRow((i+1), SubjectListOneData.data);
								zui5_hrxx_actapp2.common.Common.oSubjectList.setUserData((i+1), "Pernr", oData.results[i].Pernr);
							}
						}
					}
				},
		...
		zui5_hrxx_actapp2.common.Common.oSubjectList.stopFastOperations();
		console.log("Row Length : " + SubjectListData.rows.length);
		
		if(zui5_hrxx_actapp2.common.Common.oSubjectList && SubjectListData.rows.length > 0) {
	//		zui5_hrxx_actapp2.common.Common.oSubjectList.parse(SubjectListData,"json");	
			
			for(var i=1; i<zui5_hrxx_actapp2.common.Common.oSubjectList.getColumnsNum(); i++) {
				zui5_hrxx_actapp2.common.Common.oSubjectList.adjustColumnSize(i);	
			}

			for(var r=0; r<zui5_hrxx_actapp2.common.Common.oSubjectList.getRowsNum(); r++) {
				if((r % 2) == 0) {
					zui5_hrxx_actapp2.common.Common.oSubjectList.setRowspan((r+1),0,2);
					zui5_hrxx_actapp2.common.Common.oSubjectList.setRowspan((r+1),1,2);
					zui5_hrxx_actapp2.common.Common.oSubjectList.setRowspan((r+1),2,2);
					zui5_hrxx_actapp2.common.Common.oSubjectList.setRowspan((r+1),3,2);
				}
			}
...
//							}							
	...
	onAfterRenderingTable : function(oController) {
		...
		zui5_hrxx_actapp2.common.Common.oSubjectList = new dhtmlXGridObject(oController.PAGEID + "_SubjectList");
		zui5_hrxx_actapp2.common.Common.oSubjectList.setImagePath("/sap/bc/ui5_ui5/sap/ZL2P01UI59000/css/imgs/dhxgrid_web/");
		...
		zui5_hrxx_actapp2.common.Common.oSubjectList.setHeader(header.substring(0, header.length - 1));
		zui5_hrxx_actapp2.common.Common.oSubjectList.setInitWidths(InitWidths.substring(0, InitWidths.length - 1));
		zui5_hrxx_actapp2.common.Common.oSubjectList.setColAlign(ColAlign.substring(0, ColAlign.length - 1));
		zui5_hrxx_actapp2.common.Common.oSubjectList.setColVAlign(ColVlign.substring(0, ColVlign.length - 1));
		zui5_hrxx_actapp2.common.Common.oSubjectList.setColTypes(ColTypes.substring(0, ColTypes.length - 1));
		
		zui5_hrxx_actapp2.common.Common.oSubjectList.attachEvent("onCheckbox", zui5_hrxx_actapp2.common.Common.doOnCheck);
		
		zui5_hrxx_actapp2.common.Common.oSubjectList.init();
		
		zui5_hrxx_actapp2.common.Common.oSubjectList.enableSmartRendering(true);
	},
