...
sap.ui.controller("zui5_hrxx_actapp2.ActAppRequest", {
...
//	}
	...
	onBeforeShow : function(oEvent) {
		...
		if(oEvent) {
			...
			zui5_hrxx_actapp2.fragment.AttachFileAction.oController = this;
			
			zui5_hrxx_actapp2.fragment.AttachFileAction.setAttachFile(this);
			
			zui5_hrxx_actapp2.fragment.AttachFileAction.refreshAttachFileList(this);
...
//	                + "Reqno='" + encodeURI(oController._vReqno) + "',"
	...
	navToBack : function(oEvent){
		var oView = sap.ui.getCore().byId("zui5_hrxx_actapp2.ActAppRequest");
	...
	onPressSave : function(oEvent){
		...
		if(oNotet.getValue() == "") {
			...
			sap.m.MessageBox.alert(oBundleText.getText("MSG_INPUT_ADDINFO1"));
			return false;
		}
		...
		if(vActionAppGrouping && vActionAppGrouping.length) {
			for(var i=0; i<vActionAppGrouping.length; i++) {
				...
				if(vActionAppGrouping[i].Grpn1 == "" || vActionAppGrouping[i].Grpn1 == "0000") {
					sap.m.MessageBox.alert((i+1) + oBundleText.getText("MSG_SELECT_GROUPING"));
					return false;
				} //Grpt1
				
				if(vActionAppGrouping[i].Grpt1E == true && vActionAppGrouping[i].Grpt1 == "") {
					sap.m.MessageBox.alert(oBundleText.getText("MSG_ACTION_GROUPNAME_INPUT").replace("$NO$", (i+1)));
					return false;
				}
...
//	          + "Reqno='" + encodeURI(oController._vReqno) + "',"
	...
	onPressRequest : function(oEvent){
		...
		if(oController.getNoticeCheck(oController)) {
			if(!oController._NoticeDialog) {
				oController._NoticeDialog = sap.ui.jsfragment("zui5_hrxx_actapp2.fragment.Notice1Dialog", oController);
	...
	onProcessRequest : function(oEvent){
		...
		if(!oController.BusyDialog) {
			...
			oController.BusyDialog.addContent(new sap.m.BusyIndicator({text : oBundleText.getText("MSG_ACTAPP_REQUEST")}));
