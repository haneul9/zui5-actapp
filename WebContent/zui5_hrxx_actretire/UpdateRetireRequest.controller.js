...
sap.ui.controller("zui5_hrxx_actretire.UpdateRetireRequest", {
...
*/
	...
	onBeforeShow: function(oEvent) {		
		...
		oIconTabBar1.setVisible(true);
		oIconTabBar2.setVisible(true);
		oIconTabBar3.setVisible(true);
		oIconTabBar4.setVisible(true);
		oIconTabBar5.setVisible(true);
		oIconTabBar6.setVisible(true);
					...
					function(oData, oResponse) {					
						if(oData) {
							...
							if(oData.Reexp == "X") {
								...
								if(oIconTabBar3) oIconTabBar3.setVisible(true);
							}
							if(oData.Seexp == "X") {
...
//	}
	...
	navToBack : function(oEvent) {
		var oView = sap.ui.getCore().byId("zui5_hrxx_actretire.UpdateRetireRequest");
	...
	handleIconTabBarSelect : function(oEvent) {
		...
		var oRCHdono = sap.ui.getCore().byId(oController.PAGEID + "_RC_Hndno");
	...
	onPressSurveyStart : function(oEvent) {
		...
		if(!oController._SurveyDialog) {
			oController._SurveyDialog = sap.ui.jsfragment("zui5_hrxx_actretire.fragment.RetireSurveyDialog", oController);
...
//			}
	...
	onPressPreview : function(oEvent) {
		...
		if(!oController._PreviewDialog) {
			oController._PreviewDialog = sap.ui.jsfragment("zui5_hrxx_actretire.fragment.RetireRequestPreview1", oController);
	...
	onSaveDocUpload : function(vActty) {
		...
		var UploadFinshProcess = function() {
			...
			if(vActty == "DC") {
				...
				try {
					...
					sap.ui.getCore().getEventBus().publish("nav", "to", {
					      id : "zui5_hrxx_actretire.RetireRequestList",
					      data : {
					      }
					});
		...
		var SaveFileProcess = function() {
			...
			try {
				...
				updateData.Bankn = oRCBankn.getValue();
				updateData.Irpno = oRCIrpno.getValue();
	...
	onSelectStaff : function(oEvent) {
		...
		if(!oController._StaffDialog) {
			oController._StaffDialog = sap.ui.jsfragment("zui5_hrxx_actretire.fragment.CheckStaffDialog", oController);
...
//		var oEname = sap.ui.getCore().byId(oController.PAGEID + "_Ename");
	...
	makeDocEmailHtml : function(oController) {
		...
		var html_url = "/sap/bc/ui5_ui5/sap/zhrxx_common/emailhtml/retire_doc_email.html";
