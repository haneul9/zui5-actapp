...
sap.ui.jsview("zui5_hrxx_actpersonalid.AppMain", {
	...
	*/ 
	getControllerName : function() {
		return "zui5_hrxx_actpersonalid.AppMain";
	},
	...
	*/ 
	createContent : function(oController) {
		...
		this.app = new sap.m.App({initialPage: "zui5_hrxx_actpersonalid.PersonalidRequestList"});		
		this.app.addPage(sap.ui.jsview("zui5_hrxx_actpersonalid.PersonalidRequestList", "zui5_hrxx_actpersonalid.PersonalidRequestList"));
	...
	loadModel : function() {
		...
		var sServiceURL1 = this.getUrl("/sap/opu/odata/sap/ZHRXX_PERSONALID_SRV/");
		...
		sap.ui.getCore().setModel(oModel,"ZHRXX_PERSONALID_SRV");
		...
		// 국가 코드 리스트
 		var oIdcotList = new sap.ui.model.json.JSONModel();
 		sap.ui.getCore().setModel(oIdcotList, "IdcotList"); 
 		var vCode = {
			IdcotCode : []         // 국가
 		};
 		// 국가코드 
 		oCommonModel.read("/EmpCodeListSet/?$filter=Field eq 'Land1' and PersaNc eq 'X'", 
 					null, 
 					null, 
 					false, 
 					function(oData, oResponse) {
 						...
 						if(oData && oData.results.length) {
 							for(i=0; i<oData.results.length; i++) {
 								vCode.IdcotCode.push(oData.results[i]);
 							}
 						}
 					},
 					function(oResponse) {
		...
		// 신분증 유형 리스트
 		var oIctypList = new sap.ui.model.json.JSONModel();
 		sap.ui.getCore().setModel(oIctypList, "IctypList"); 
 		var vCode = {
			IctypCode : []         // 신분증
 		};
 		// 신분증코드 
 		oCommonModel.read("/EmpCodeListSet?$filter=Field eq 'Ictyp' and PersaNc eq 'X'", 
 					null, 
 					null, 
 					false, 
 					function(oData, oResponse) {
 						...
 						if(oData && oData.results.length) {
 							for(i=0; i<oData.results.length; i++) {
 								vCode.IctypCode.push(oData.results[i]);
 							}
 						}
 					},
 					function(oResponse) {
 		...
 		oIctypList.setData(vCode);
 		
		//신분증 신청 리스트
		var mPIDRegistrationList = new sap.ui.model.json.JSONModel();
		sap.ui.getCore().setModel(mPIDRegistrationList, "PIDRegistrationList");
		mPIDRegistrationList.setSizeLimit(3000);
