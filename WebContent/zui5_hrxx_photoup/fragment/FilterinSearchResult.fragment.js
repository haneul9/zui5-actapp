sap.ui.jsfragment("zui5_hrxx_photoup.fragment.FilterinSearchResult", {
...
//        });
		...
		var vMtypeColumns = {MtypeCollection : [
                                        	{id : "0", label : oBundleText.getText("ALL_ENTRY"), visible : true},    
		                        		    {id : "S", label : oBundleText.getText("ACTION_COMPLETE_STATUS2"), visible : true},
		                        		    {id : "E", label : oBundleText.getText("ACTION_COMPLETE_STATUS3") , visible : true}
		                        		]};
		...
		var mMtypeModel = new sap.ui.model.json.JSONModel();
		mMtypeModel.setData(vMtypeColumns);
		...
		oFilterLayout.addContent(
			new sap.m.Toolbar({
				...
				content : [new sap.m.Label({
					 			text : oBundleText.getText( "MTYPE")
						   }).addStyleClass("L2P13Font"),
				           new sap.m.Select(oController.PAGEID + "_FSR_Column1", {
								items : {
									path : "/MtypeCollection",
									template : new sap.ui.core.Item ({key : "{id}", text : "{label}"})}
						   }).setModel(mMtypeModel),
				           ]
