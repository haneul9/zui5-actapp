...
*/
	...
	onAfterShow: function(evt) {
		...
		if(ReadyHtmlEditor == false) {
			if(!oController.BusyDialog) {
				oController.BusyDialog = new sap.m.Dialog({showHeader : false}); 
				oController.BusyDialog.addContent(new sap.m.BusyIndicator({text : "Loading Html Editor"}));
		...
		if(this._vDocst == "200" || this._vDocst == "300") {
			...
			var modify1 = function() {
				if(ReadyHtmlEditor == true) {
					document.getElementById("htmleditor1").contentWindow.modifyContent(mActionReqList.getProperty(oController._vContext + "/Htmlc"));
					if(oController.BusyDialog && oController.BusyDialog.isOpen()) {
						oController.BusyDialog.close();
					};
				} else {
					setTimeout(modify1, 500);
				}
			};
			...
			var modify1 = function() {
				if(ReadyHtmlEditor == true) {
					document.getElementById("htmleditor1").contentWindow.modifyContent(" ");
					if(oController.BusyDialog && oController.BusyDialog.isOpen()) {
						oController.BusyDialog.close();
					};
				} else {
...
//	}
	...
	onPressSave : function(oEvent) {
		...
		document.getElementById("htmleditor1").contentWindow.saveContent();
		//Editor.save();
		
		if(HtmlEditorContent == "") {
			sap.m.MessageBox.alert(oBundleText.getText("MSG_ANNOUNCE1"));
			return;
		}
		...
		createData.Htmlc = HtmlEditorContent;
